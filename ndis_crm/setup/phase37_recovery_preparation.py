...
def install():
    ...
    create_recovery_preparation_doctypes()
    create_custom_fields_phase37()
    create_form_scripts()
    frappe.clear_cache()
    frappe.db.commit()
    print("NDIS CRM Phase 37 Recovery Preparation bridge installed successfully.")
...
def ensure_required_doctypes():
    ...
    if missing:
        frappe.throw("Missing required Phase 37 DocTypes: " + ", ".join(missing))

    print("Required Phase 37 DocTypes found.")
...
def create_recovery_preparation_doctypes():
    create_recovery_preparation_line()
    create_recovery_preparation_run()


def create_recovery_preparation_line():
    fields = [
        ...
        make_field("Recovery Preparation Source Key", "recovery_preparation_source_key", "Data", read_only=1),
        ...
        make_field("Variance / Payment Values", "variance_payment_values_section", "Section Break"),
        ...
        make_field("Recovery Preparation", "recovery_preparation_section", "Section Break"),
        make_field("Proposed Recovery Amount", "proposed_recovery_amount", "Currency", in_list_view=1),
        make_field("Recovery Type", "recovery_type", "Select", options="Overpayment Recovery\nDuplicate Payment Recovery\nRejected Claim Recovery\nShort Payment Recovery\nParticipant Debt Review\nPlan Manager Follow-Up\nManual Recovery Review", in_list_view=1),
        make_field("Recovery Route", "recovery_route", "Select", options="Accounts Recovery Review\nPlan Manager Follow-Up\nParticipant Debt Review\nProvider Correction\nNDIA / Portal Correction\nInternal Correction Review", in_list_view=1),
        ...
        make_field("Recovery Preparation Source Ready", "recovery_preparation_source_ready", "Check", default="0"),
        make_field("Recovery Review Complete", "recovery_review_complete", "Check", default="0"),
        make_field("Recovery Decision Authorized", "recovery_decision_authorized", "Check", default="0"),
        make_field("Recovery Case Creation Authorized", "recovery_case_creation_authorized", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Journal Entry Authorized", "journal_entry_authorized", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Manual GL Authorized", "manual_gl_authorized", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Payment Entry Authorized", "payment_entry_authorized", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Sales Invoice Authorized", "sales_invoice_authorized", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Adjustment Authorized", "adjustment_authorized", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Bank Reconciliation Authorized", "bank_reconciliation_authorized", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Recovery Preparation Hold", "recovery_preparation_hold", "Check", default="1", in_list_view=1),
        make_field("Recovery Preparation Hold Reason", "recovery_preparation_hold_reason", "Small Text"),

        make_field("Line Status", "line_status_section", "Section Break"),
        make_field("Line Ready for Recovery Preparation", "line_ready_for_recovery_preparation", "Check", default="0", in_list_view=1),
        make_field("Recovery Preparation Line Status", "recovery_preparation_line_status", "Select", options="Draft\nReady\nApproved\nRecovery Prepared\nReturned\nCancelled", default="Draft", in_list_view=1),
        make_field("Notes", "notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=RECOVERY_PREPARATION_LINE,
        fields=fields,
        istable=1,
    )


def create_recovery_preparation_run():
    fields = [
        make_field("Series", "naming_series", "Select", options="NDIS-RECOVERY-PREP-.YYYY.-.#####", default="NDIS-RECOVERY-PREP-.YYYY.-.#####", reqd=1),

        make_field("Status", "status_section", "Section Break"),
        make_field("Status", "status", "Select", options="Draft\nIn Review\nReady for Recovery Preparation\nRecovery Preparation Approved\nRecovery Prepared\nReturned to Source Review\nCancelled", default="Draft", in_list_view=1),
        make_field("Readiness %", "readiness_percent", "Percent", read_only=1, in_list_view=1),
        make_field("Recovery Preparation Run Ready", "recovery_preparation_run_ready", "Check", read_only=1, in_list_view=1),
        make_field("No Recovery Candidate Found", "no_recovery_candidate_found", "Check", default="0", read_only=1),
        make_field("Recovery Source Type", "recovery_source_type", "Select", options="Phase 31 Variance Rejection Review\nPhase 36 Write Off Finalisation", read_only=1),
        make_field("Recovery Preparation Completion Allowed", "recovery_preparation_completion_allowed", "Check", default="0"),
        make_field("Recovery Case Creation Allowed", "recovery_case_creation_allowed", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Journal Entry Creation Allowed", "journal_entry_creation_allowed", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Manual GL Creation Allowed", "manual_gl_creation_allowed", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Payment Entry Creation Allowed", "payment_entry_creation_allowed", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Sales Invoice Creation Allowed", "sales_invoice_creation_allowed", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Adjustment Creation Allowed", "adjustment_creation_allowed", "Check", default="0", description="Blocked in Phase 37."),
        make_field("Bank Reconciliation Allowed", "bank_reconciliation_allowed", "Check", default="0", description="Blocked in Phase 37."),

        make_field("Summary", "summary_section", "Section Break"),
        make_field("Recovery Preparation Line Count", "recovery_preparation_line_count", "Int", read_only=1),
        make_field("Proposed Recovery Amount Total", "proposed_recovery_amount_total", "Currency", read_only=1),
        make_field("Recovery Preparation Ready Count", "recovery_preparation_ready_count", "Int", read_only=1),
        make_field("Recovery Preparation Hold Count", "recovery_preparation_hold_count", "Int", read_only=1),
        make_field("Overpayment Recovery Count", "overpayment_recovery_count", "Int", read_only=1),
        make_field("Rejected Claim Recovery Count", "rejected_claim_recovery_count", "Int", read_only=1),
        make_field("Short Payment Recovery Count", "short_payment_recovery_count", "Int", read_only=1),
        make_field("Manual Recovery Review Count", "manual_recovery_review_count", "Int", read_only=1),
    ...
    fields += [
        ...
        make_field("Recovery Preparation Owner", "recovery_preparation_owner", "Link", options="User", in_list_view=1),
        make_field("Variance Review Owner", "variance_review_owner", "Link", options="User"),
        make_field("Write Off Finalisation Owner", "write_off_finalisation_owner", "Link", options="User"),
        ...
        make_field("Recovery Preparation Lines", "recovery_preparation_lines_section", "Section Break"),
        make_field("Recovery Preparation Lines", "recovery_preparation_lines", "Table", options=RECOVERY_PREPARATION_LINE),

        make_field("Notes", "notes_section", "Section Break"),
        make_field("Recovery Preparation Notes", "recovery_preparation_notes", "Small Text"),
        make_field("Returned / Blocker Notes", "returned_notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=RECOVERY_PREPARATION_RUN,
        fields=fields,
...
def create_custom_fields_phase37():
    deal_fields = [
        {
            "fieldname": "recovery_preparation_section",
            "label": "NDIS Recovery Preparation Run",
            "fieldtype": "Section Break",
            "insert_after": "write_off_finalisation_ready",
        },
        {
            "fieldname": "ndis_recovery_preparation_run_required",
            "label": "Recovery Preparation Required Before Active Deal",
            "fieldtype": "Check",
            "default": "0",
            "insert_after": "recovery_preparation_section",
            "description": "Optional guard only. Recovery preparation normally happens after variance review or write-off finalisation.",
        },
        {
            "fieldname": "ndis_recovery_preparation_run",
            "label": "NDIS CRM Recovery Preparation Run",
            "fieldtype": "Link",
            "options": RECOVERY_PREPARATION_RUN,
            "read_only": 1,
            "insert_after": "ndis_recovery_preparation_run_required",
        },
        {
            "fieldname": "recovery_preparation_status",
            "label": "Recovery Preparation Status",
            "fieldtype": "Data",
            "read_only": 1,
            "insert_after": "ndis_recovery_preparation_run",
        },
        {
            "fieldname": "recovery_preparation_ready",
            "label": "Recovery Preparation Ready",
            "fieldtype": "Check",
            "read_only": 1,
            "insert_after": "recovery_preparation_status",
        },
    ]
    ...
    print("Created / updated Phase 37 recovery preparation chain custom fields.")
...
def _phase37_deal_actions():
    return r'''
// NDIS CRM Phase 37 Deal Actions
{
  label: "Create Recovery Preparation Run",
  onClick: () => {
    call("ndis_crm.phase37_recovery_preparation.create_recovery_preparation_run_from_crm_deal", {
      deal: doc.name
    }).then((data) => {
      if (data && data.name) {
        createToast({
          title: data.created ? "Recovery Preparation Run Created" : "Existing Recovery Preparation Run Found",
          icon: "check",
          iconClasses: "text-green-600",
        })
        window.open(`/app/ndis-crm-recovery-preparation-run/${data.name}`, "_blank")
      }
    })
  }
},
{
  label: "Open Recovery Preparation Run",
  onClick: () => {
    if (doc.ndis_recovery_preparation_run) {
      window.open(`/app/ndis-crm-recovery-preparation-run/${doc.ndis_recovery_preparation_run}`, "_blank")
    } else {
      createToast({ title: "No Recovery Preparation Run linked yet", icon: "info" })
    }
  }
}
'''.strip()
...
def _deal_script():
    try:
        from ndis_crm.setup.phase36_write_off_finalisation import _deal_script as phase36_deal_script
        script = phase36_deal_script()
    except Exception:
        script = '''
...
'''.strip()
    ...
    if "NDIS CRM Phase 37 Deal Actions" in script:
        return script

    return _append_before_last(script, "\n    ]", _phase37_deal_actions())
...
frappe.ui.form.on("NDIS CRM Variance Rejection Review Run", {
    refresh(frm) {
        ...
        frm.add_custom_button(__("Create Recovery Preparation Run"), function () {
            frappe.call({
                method: "ndis_crm.phase37_recovery_preparation.create_recovery_preparation_run_from_variance_review_run",
