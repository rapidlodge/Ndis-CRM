...
def install():
    ...
    create_recovery_outcome_action_preparation_doctypes()
    create_custom_fields_phase47()
    create_form_scripts()
    frappe.clear_cache()
    frappe.db.commit()
    print("NDIS CRM Phase 47 Recovery Outcome Action Preparation bridge installed successfully.")
...
def repair_partial_crm_deal_phase47_fields():
    expected = {
    ...
    for fieldname, values in expected.items():
        ...
        if changed:
            ...
            print(f"Repaired CRM Deal Phase 47 field: {custom_field_name}")
...
def ensure_required_doctypes():
    ...
    if missing:
        frappe.throw("Missing required Phase 47 DocTypes: " + ", ".join(missing))

    print("Required Phase 47 DocTypes found.")
...
def create_recovery_outcome_action_preparation_doctypes():
    create_recovery_outcome_action_preparation_line()
    create_recovery_outcome_action_preparation_run()


def create_recovery_outcome_action_preparation_line():
    fields = [
        make_field("Source", "source_section", "Section Break"),
        make_field("Recovery Outcome Action Source Key", "recovery_outcome_action_source_key", "Data", read_only=1),

        make_field("Outcome Snapshot", "outcome_snapshot_section", "Section Break"),
        make_field("Outcome Status Snapshot", "outcome_status_snapshot", "Data", in_list_view=1, read_only=1),
        ...
        make_field("Action Preparation", "action_preparation_section", "Section Break"),
        make_field("Recovery Outcome Action Source Ready", "recovery_outcome_action_source_ready", "Check", default="0"),
        make_field("Action Type", "action_type", "Select", options="No Response Follow-Up Preparation\nResponse Review Preparation\nPayment Promise Monitoring Preparation\nEvidence Follow-Up Preparation\nWrong Contact Correction Preparation\nDispute Review Preparation\nEscalation Review Preparation\nClosure Review Preparation\nManual Review Preparation", in_list_view=1),
        ...
        make_field("Action Review Complete", "action_review_complete", "Check", default="0"),
        make_field("Action Decision Recorded", "action_decision_recorded", "Check", default="0"),
        make_field("Action Authorized", "action_authorized", "Check", default="0"),

        make_field("Blocked Downstream Controls", "blocked_downstream_section", "Section Break"),
        make_field("Communication Creation Authorized", "communication_creation_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Email Send Authorized", "email_send_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Event Creation Authorized", "event_creation_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("ToDo Creation Authorized", "todo_creation_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Task Creation Authorized", "task_creation_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Recovery Case Creation Authorized", "recovery_case_creation_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Journal Entry Authorized", "journal_entry_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Manual GL Authorized", "manual_gl_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Payment Entry Authorized", "payment_entry_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Sales Invoice Authorized", "sales_invoice_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Adjustment Authorized", "adjustment_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Bank Reconciliation Authorized", "bank_reconciliation_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Claim Batch Authorized", "claim_batch_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Claim Line Authorized", "claim_line_authorized", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Remittance Import Authorized", "remittance_import_authorized", "Check", default="0", description="Blocked in Phase 47."),

        make_field("Line Status", "line_status_section", "Section Break"),
        make_field("Recovery Outcome Action Hold", "recovery_outcome_action_hold", "Check", default="1", in_list_view=1),
        make_field("Recovery Outcome Action Hold Reason", "recovery_outcome_action_hold_reason", "Small Text"),
        make_field("Line Ready for Recovery Outcome Action Preparation", "line_ready_for_recovery_outcome_action_preparation", "Check", default="0", in_list_view=1),
        make_field("Recovery Outcome Action Preparation Line Status", "recovery_outcome_action_preparation_line_status", "Select", options="Draft\nReady\nApproved\nAction Prepared\nReturned\nCancelled", default="Draft", in_list_view=1),
        make_field("Notes", "notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=RECOVERY_OUTCOME_ACTION_PREPARATION_LINE,
        fields=fields,
        istable=1,
    )


def create_recovery_outcome_action_preparation_run():
    fields = [
        make_field("Series", "naming_series", "Select", options="NDIS-RECOVERY-ACTION-PREP-.YYYY.-.#####", default="NDIS-RECOVERY-ACTION-PREP-.YYYY.-.#####", reqd=1),

        make_field("Status", "status_section", "Section Break"),
        make_field("Status", "status", "Select", options="Draft\nIn Review\nReady for Action Preparation\nAction Preparation Approved\nActions Prepared\nReturned to Outcome Capture\nCancelled", default="Draft", in_list_view=1),
        make_field("Readiness %", "readiness_percent", "Percent", read_only=1, in_list_view=1),
        make_field("Recovery Outcome Action Preparation Run Ready", "recovery_outcome_action_preparation_run_ready", "Check", read_only=1, in_list_view=1),
        make_field("No Recovery Outcome Action Candidate Found", "no_recovery_outcome_action_candidate_found", "Check", default="0", read_only=1),

        make_field("Action Preparation Completion Allowed", "action_preparation_completion_allowed", "Check", default="0"),
        make_field("Communication Creation Allowed", "communication_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Email Send Allowed", "email_send_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Event Creation Allowed", "event_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("ToDo Creation Allowed", "todo_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Task Creation Allowed", "task_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Recovery Case Creation Allowed", "recovery_case_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Journal Entry Creation Allowed", "journal_entry_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Manual GL Creation Allowed", "manual_gl_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Payment Entry Creation Allowed", "payment_entry_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Sales Invoice Creation Allowed", "sales_invoice_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Adjustment Creation Allowed", "adjustment_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Bank Reconciliation Allowed", "bank_reconciliation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Claim Batch Creation Allowed", "claim_batch_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Claim Line Creation Allowed", "claim_line_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),
        make_field("Remittance Import Creation Allowed", "remittance_import_creation_allowed", "Check", default="0", description="Blocked in Phase 47."),

        make_field("Summary", "summary_section", "Section Break"),
        make_field("Recovery Outcome Action Preparation Line Count", "recovery_outcome_action_preparation_line_count", "Int", read_only=1),
        make_field("Recovery Outcome Action Amount Total", "recovery_outcome_action_amount_total", "Currency", read_only=1),
        make_field("Recovery Outcome Action Ready Count", "recovery_outcome_action_ready_count", "Int", read_only=1),
        make_field("Recovery Outcome Action Hold Count", "recovery_outcome_action_hold_count", "Int", read_only=1),
        make_field("No Response Follow Up Count", "no_response_follow_up_count", "Int", read_only=1),
        make_field("Payment Promise Monitoring Count", "payment_promise_monitoring_count", "Int", read_only=1),
        make_field("Evidence Follow Up Count", "evidence_follow_up_count", "Int", read_only=1),
        make_field("Wrong Contact Correction Count", "wrong_contact_correction_count", "Int", read_only=1),
        ...
        make_field("Closure Review Count", "closure_review_count", "Int", read_only=1),
        make_field("Manual Review Count", "manual_review_count", "Int", read_only=1),
        make_field("Action Prepared Count", "action_prepared_count", "Int", read_only=1),
    ...
    fields += [
        ...
        make_field("NDIS CRM Recovery Communication Outcome Capture Run", "recovery_communication_outcome_capture_run", "Link", options=RECOVERY_COMMUNICATION_OUTCOME_CAPTURE_RUN, reqd=1, in_list_view=1),
        ...
        make_field("Recovery Outcome Action Preparation Owner", "recovery_outcome_action_preparation_owner", "Link", options="User", in_list_view=1),
        make_field("Recovery Communication Outcome Capture Owner", "recovery_communication_outcome_capture_owner", "Link", options="User"),
        ...
        make_field("Recovery Outcome Action Preparation Lines", "recovery_outcome_action_preparation_lines_section", "Section Break"),
        make_field("Recovery Outcome Action Preparation Lines", "recovery_outcome_action_preparation_lines", "Table", options=RECOVERY_OUTCOME_ACTION_PREPARATION_LINE),

        make_field("Notes", "notes_section", "Section Break"),
        make_field("Recovery Outcome Action Preparation Notes", "recovery_outcome_action_preparation_notes", "Small Text"),
        make_field("Returned / Blocker Notes", "returned_notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=RECOVERY_OUTCOME_ACTION_PREPARATION_RUN,
        fields=fields,
...
def create_custom_fields_phase47():
    deal_fields = [
        {
            "fieldname": "recovery_outcome_action_preparation_section",
            "label": "NDIS Recovery Outcome Action Preparation Run",
            "fieldtype": "Section Break",
            "insert_after": "recovery_communication_outcome_capture_ready",
        },
        {
            "fieldname": "ndis_recovery_outcome_action_preparation_run_required",
            "label": "Recovery Outcome Action Preparation Required Before Active Deal",
            "fieldtype": "Check",
            "default": "0",
            "insert_after": "recovery_outcome_action_preparation_section",
        },
        {
            "fieldname": "ndis_recovery_outcome_action_preparation_run",
            "label": "NDIS CRM Recovery Outcome Action Preparation Run",
            "fieldtype": "Small Text",
            "read_only": 1,
            "insert_after": "ndis_recovery_outcome_action_preparation_run_required",
        },
        {
            "fieldname": "recovery_outcome_action_preparation_status",
            "label": "Recovery Outcome Action Preparation Status",
            "fieldtype": "Small Text",
            "read_only": 1,
            "insert_after": "ndis_recovery_outcome_action_preparation_run",
        },
        {
            "fieldname": "recovery_outcome_action_preparation_ready",
            "label": "Recovery Outcome Action Preparation Ready",
            "fieldtype": "Check",
            "read_only": 1,
            "insert_after": "recovery_outcome_action_preparation_status",
        },
    ]
    ...
    shared_fields = [
        ...
        },
        {
            ...
            "label": "NDIS CRM Recovery Outcome Action Preparation Run",
            "fieldtype": "Link",
            "options": RECOVERY_OUTCOME_ACTION_PREPARATION_RUN,
            "read_only": 1,
            "insert_after": "recovery_outcome_action_preparation_section",
        },
        {
    ...
    repair_partial_crm_deal_phase47_fields()
    safe_create_custom_fields(custom_fields)
    print("Created / updated Phase 47 recovery outcome action preparation chain custom fields.")
...
def _phase47_deal_actions():
    return r'''
// NDIS CRM Phase 47 Deal Actions
{
  label: "Create Recovery Outcome Action Preparation Run",
  onClick: () => {
    call("ndis_crm.phase47_recovery_outcome_action_preparation.create_recovery_outcome_action_preparation_run_from_crm_deal", {
      deal: doc.name
    }).then((data) => {
      if (data && data.name) {
        createToast({
          title: data.created ? "Recovery Outcome Action Preparation Run Created" : "Existing Recovery Outcome Action Preparation Run Found",
          icon: "check",
          iconClasses: "text-green-600",
        })
        window.open(`/app/ndis-crm-recovery-outcome-action-preparation-run/${data.name}`, "_blank")
      }
    })
  }
},
{
  label: "Open Recovery Outcome Action Preparation Run",
  onClick: () => {
    if (doc.ndis_recovery_outcome_action_preparation_run) {
      window.open(`/app/ndis-crm-recovery-outcome-action-preparation-run/${doc.ndis_recovery_outcome_action_preparation_run}`, "_blank")
    } else {
      createToast({ title: "No Recovery Outcome Action Preparation Run linked yet", icon: "info" })
    }
  }
}
'''.strip()
...
def _deal_script():
    try:
        from ndis_crm.setup.phase46_recovery_communication_outcome_capture import _deal_script as phase46_deal_script

        script = phase46_deal_script()
    except Exception:
        script = '''
...
'''.strip()
    ...
    if "NDIS CRM Phase 47 Deal Actions" in script:
        return script

    return _append_before_last(script, "\n    ]", _phase47_deal_actions())
...
frappe.ui.form.on("NDIS CRM Recovery Communication Outcome Capture Run", {
    refresh(frm) {
        ...
        frm.add_custom_button(__("Create Recovery Outcome Action Preparation Run"), function () {
            frappe.call({
                method: "ndis_crm.phase47_recovery_outcome_action_preparation.create_recovery_outcome_action_preparation_run_from_outcome_capture_run",
                args: { recovery_communication_outcome_capture_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Creating recovery outcome action preparation run...")
            }).then((r) => {
                if (r.message && r.message.name) {
                    frappe.show_alert({ message: r.message.message || __("Recovery Outcome Action Preparation Run created"), indicator: "green" });
                    frm.reload_doc();
                    frappe.set_route("Form", "NDIS CRM Recovery Outcome Action Preparation Run", r.message.name);
                }
            });
        }, __("Actions"));

        if (frm.doc.ndis_recovery_outcome_action_preparation_run) {
            frm.add_custom_button(__("Open Recovery Outcome Action Preparation Run"), function () {
                frappe.set_route("Form", "NDIS CRM Recovery Outcome Action Preparation Run", frm.doc.ndis_recovery_outcome_action_preparation_run);
            }, __("Open"));
        }
    }
});
...
frappe.ui.form.on("NDIS CRM Recovery Outcome Action Preparation Run", {
    refresh(frm) {
        ...
        frm.add_custom_button(__("Generate Action Lines"), function () {
            frappe.call({
                method: "ndis_crm.phase47_recovery_outcome_action_preparation.generate_recovery_outcome_action_preparation_lines",
                args: { recovery_outcome_action_preparation_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Generating action preparation lines...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Action lines generated"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));

        frm.add_custom_button(__("Validate Action Readiness"), function () {
            frappe.call({
                method: "ndis_crm.phase47_recovery_outcome_action_preparation.validate_recovery_outcome_action_preparation_readiness",
                args: { recovery_outcome_action_preparation_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Validating action readiness...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Action readiness validated"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));

        frm.add_custom_button(__("Mark Ready for Action Preparation"), function () {
            frappe.call({
                method: "ndis_crm.phase47_recovery_outcome_action_preparation.mark_ready_for_recovery_outcome_action_preparation",
                args: { recovery_outcome_action_preparation_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Marking ready for action preparation...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Ready for action preparation"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));

        frm.add_custom_button(__("Approve Action Preparation Run"), function () {
            frappe.call({
                method: "ndis_crm.phase47_recovery_outcome_action_preparation.approve_recovery_outcome_action_preparation_run",
                args: { recovery_outcome_action_preparation_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Approving action preparation run...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Action Preparation Run approved"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));

        frm.add_custom_button(__("Complete Action Preparation"), function () {
            frappe.confirm(
                __("This prepares next-action decisions only. It will not create Communication, Email Queue, Event, ToDo, Task, Recovery Case, Journal Entry, GL Entry, Payment Entry, Sales Invoice, adjustment, bank reconciliation, Claim Batch, Claim Line, or Remittance Import."),
                function () {
                    frappe.call({
                        method: "ndis_crm.phase47_recovery_outcome_action_preparation.complete_recovery_outcome_action_preparation",
                        args: { recovery_outcome_action_preparation_run: frm.doc.name },
                        freeze: true,
                        freeze_message: __("Completing action preparation...")
                    }).then((r) => {
                        if (r.message) {
                            frappe.show_alert({ message: r.message.message || __("Action preparation completed"), indicator: "green" });
                            frm.reload_doc();
                        }
                    });
                }
        ...
        if (frm.doc.recovery_communication_outcome_capture_run) {
            frm.add_custom_button(__("Open Outcome Capture Run"), function () {
                frappe.set_route("Form", "NDIS CRM Recovery Communication Outcome Capture Run", frm.doc.recovery_communication_outcome_capture_run);
            }, __("Open"));
        }
