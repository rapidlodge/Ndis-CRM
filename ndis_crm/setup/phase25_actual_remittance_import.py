...
def install():
	...
	create_actual_remittance_import_doctypes()
	create_custom_fields_phase25()
	create_form_scripts()
	frappe.clear_cache()
	frappe.db.commit()
	print("NDIS CRM Phase 25 Actual Remittance Import draft bridge installed successfully.")
...
def ensure_required_doctypes():
	...
	if missing:
		frappe.throw("Missing required Phase 25 DocTypes: " + ", ".join(missing))
	print("Required Phase 25 DocTypes found.")
...
def create_actual_remittance_import_doctypes():
	create_actual_remittance_import_line()
	create_actual_remittance_import_run()


def create_actual_remittance_import_line():
	support_type, support_options = optional_link("NDIS Support Item")
	service_type, service_options = optional_link("NDIS Service Type")
	budget_type, budget_options = optional_link("NDIS Plan Budget")
	booking_type, booking_options = optional_link("NDIS Service Booking")
	house_type, house_options = optional_link("NDIS House")
	fields = [
		make_field("Source", "source_section", "Section Break"),
		make_field("Actual Remittance Source Key", "actual_remittance_source_key", "Data", read_only=1, in_list_view=1),
		...
		make_field("NDIS Remittance Import", "ndis_remittance_import", "Link", options=NDIS_REMITTANCE_IMPORT, read_only=1, in_list_view=1),
		...
		make_field("Service Line", "service_line", "Link" if doctype_exists("NDIS Service Line") else "Data", options="NDIS Service Line" if doctype_exists("NDIS Service Line") else None),
		...
		make_field("Actual Rejected Amount", "actual_rejected_amount", "Currency", default=0),
		...
		make_field("Raw Actual Remittance Payload", "raw_actual_remittance_payload", "Code", read_only=1),
		make_field("Import Controls", "import_controls_section", "Section Break"),
		make_field("Remittance Preparation Source Ready", "remittance_preparation_source_ready", "Check", default=0),
		make_field("Actual File Matched", "actual_file_matched", "Check", default=0),
		make_field("Actual Remittance Review Complete", "actual_remittance_review_complete", "Check", default=0),
		make_field("Remittance Import Mapping Ready", "remittance_import_mapping_ready", "Check", default=0),
		make_field("Remittance Import Draft Authorized", "remittance_import_draft_authorized", "Check", default=0),
		make_field("Payment Entry Authorized", "payment_entry_authorized", "Check", default=0, description="Blocked in Phase 25."),
		make_field("Journal Authorized", "journal_authorized", "Check", default=0, description="Blocked in Phase 25."),
		make_field("Write Off Authorized", "write_off_authorized", "Check", default=0, description="Blocked in Phase 25."),
		make_field("Recovery Authorized", "recovery_authorized", "Check", default=0, description="Blocked in Phase 25."),
		make_field("Actual Remittance Hold", "actual_remittance_hold", "Check", default=1, in_list_view=1),
		make_field("Actual Remittance Hold Reason", "actual_remittance_hold_reason", "Small Text"),
		make_field("Line Status", "line_status_section", "Section Break"),
		make_field("Line Ready for Remittance Import Draft", "line_ready_for_remittance_import_draft", "Check", default=0, in_list_view=1),
		make_field("Actual Remittance Line Status", "actual_remittance_line_status", "Select", options="Draft\nReady\nApproved\nDraft Remittance Import Created\nImported\nMatched\nReturned\nCancelled", default="Draft", in_list_view=1),
		make_field("Notes", "notes", "Small Text"),
	]
	create_doctype_if_missing(ACTUAL_REMITTANCE_IMPORT_LINE, fields, istable=1)


def create_actual_remittance_import_run():
	fields = [
		make_field("Series", "naming_series", "Select", options="NDIS-ACTUAL-REMIT-.YYYY.-.#####", default="NDIS-ACTUAL-REMIT-.YYYY.-.#####", reqd=1),
		make_field("Status", "status_section", "Section Break"),
		make_field("Status", "status", "Select", options="Draft\nIn Review\nReady for Remittance Import Draft Creation\nActual Remittance Import Run Approved\nDraft Remittance Import Created\nReturned to Remittance Preparation\nCancelled", default="Draft", in_list_view=1),
		make_field("Readiness %", "readiness_percent", "Percent", read_only=1, in_list_view=1),
		make_field("Actual Remittance Import Run Ready", "actual_remittance_import_run_ready", "Check", read_only=1, in_list_view=1),
		make_field("Remittance Import Draft Creation Allowed", "remittance_import_draft_creation_allowed", "Check", default=0),
		make_field("Payment Entry Creation Allowed", "payment_entry_creation_allowed", "Check", default=0, description="Blocked in Phase 25."),
		make_field("Journal Creation Allowed", "journal_creation_allowed", "Check", default=0, description="Blocked in Phase 25."),
		make_field("Write Off Creation Allowed", "write_off_creation_allowed", "Check", default=0, description="Blocked in Phase 25."),
		make_field("Recovery Creation Allowed", "recovery_creation_allowed", "Check", default=0, description="Blocked in Phase 25."),
		make_field("Uploaded Actual Remittance File", "uploaded_file_section", "Section Break"),
		make_field("Uploaded Remittance File", "uploaded_remittance_file", "Attach"),
		make_field("Uploaded File Name", "uploaded_file_name", "Data", read_only=1),
		make_field("Uploaded File SHA256", "uploaded_file_sha256", "Data", read_only=1),
		make_field("Uploaded File Line Count", "uploaded_file_line_count", "Int", read_only=1),
		make_field("Uploaded File Parsed On", "uploaded_file_parsed_on", "Datetime", read_only=1),
		make_field("Uploaded File Parsed By", "uploaded_file_parsed_by", "Link", options="User", read_only=1),
		make_field("Draft Output", "draft_output_section", "Section Break"),
		make_field("NDIS Remittance Import", "ndis_remittance_import", "Link", options=NDIS_REMITTANCE_IMPORT, read_only=1),
		...
		make_field("Actual Remittance Line Count", "actual_remittance_line_count", "Int", read_only=1),
		...
		make_field("Actual File Matched Count", "actual_file_matched_count", "Int", read_only=1),
		make_field("Remittance Import Ready Count", "remittance_import_ready_count", "Int", read_only=1),
		make_field("Draft Remittance Import Line Count", "draft_remittance_import_line_count", "Int", read_only=1),
		make_field("Actual Remittance Hold Count", "actual_remittance_hold_count", "Int", read_only=1),
		...
		make_field("Remittance Import Preparation Run", "remittance_import_preparation_run", "Link", options=REMITTANCE_IMPORT_PREP_RUN, reqd=1, in_list_view=1),
		...
		make_field("Participant", "participant_section", "Section Break"),
		...
		make_field("Claim Window Start", "claim_window_start", "Date"),
		make_field("Claim Window End", "claim_window_end", "Date"),
		...
		make_field("Plan Manager", "plan_manager", "Link", options="User"),
		make_field("Actual Payment Reference", "actual_payment_reference", "Data"),
		make_field("Actual Remittance Lines", "actual_remittance_lines_section", "Section Break"),
		make_field("Actual Remittance Lines", "actual_remittance_lines", "Table", options=ACTUAL_REMITTANCE_IMPORT_LINE),
		make_field("Readiness Notes", "readiness_notes", "Small Text", read_only=1),
		make_field("Notes", "notes", "Small Text"),
	]
	create_doctype_if_missing(ACTUAL_REMITTANCE_IMPORT_RUN, fields, autoname="naming_series:", title_field="participant_name")


def create_custom_fields_phase25():
	deal_fields = [
		make_field("Actual Remittance Import", "actual_remittance_import_section", "Section Break", insert_after="remittance_import_preparation_ready"),
		make_field("Actual Remittance Import Run Required", "ndis_actual_remittance_import_run_required", "Check", default=0),
		make_field("NDIS Actual Remittance Import Run", "ndis_actual_remittance_import_run", "Link", options=ACTUAL_REMITTANCE_IMPORT_RUN, read_only=1),
		make_field("Actual Remittance Import Status", "ndis_actual_remittance_import_status", "Data", read_only=1),
		make_field("Actual Remittance Import Ready", "ndis_actual_remittance_import_ready", "Check", read_only=1),
	]
	shared_fields = [
		make_field("Actual Remittance Import", "actual_remittance_import_section", "Section Break", insert_after="remittance_import_preparation_ready"),
		make_field("NDIS Actual Remittance Import Run", "ndis_actual_remittance_import_run", "Link", options=ACTUAL_REMITTANCE_IMPORT_RUN, read_only=1),
	...
	print("Created / updated Phase 25 actual remittance import custom fields.")
...
def create_form_scripts():
	...
	if frappe.db.exists("DocType", "Client Script"):
		upsert_doc("Client Script", "NDIS CRM Remittance Import Preparation Run Actions", {"dt": REMITTANCE_IMPORT_PREP_RUN, "view": "Form", "enabled": 1, "script": _prep_script()})
		upsert_doc("Client Script", "NDIS CRM Actual Remittance Import Run Actions", {"dt": ACTUAL_REMITTANCE_IMPORT_RUN, "view": "Form", "enabled": 1, "script": _run_script()})


def _deal_script():
	try:
		from ndis_crm.setup.phase24_remittance_import_preparation import _deal_script as previous_script
	...
	insert = r'''
      {
        label: "Create Actual Remittance Import Run",
        onClick: () => {
          call("ndis_crm.phase25_actual_remittance_import.create_actual_remittance_import_run_from_crm_deal", {
            deal: doc.name
          }).then((data) => {
            if (data && data.name) {
              createToast({
                title: data.created ? "Actual Remittance Import Run Created" : "Existing Actual Remittance Import Run Found",
                icon: "check",
                iconClasses: "text-green-600",
              })
              window.open(`/app/ndis-crm-actual-remittance-import-run/${data.name}`, "_blank")
            }
          })
        }
      },
      {
        label: "Open Actual Remittance Import Run",
        onClick: () => {
          if (doc.ndis_actual_remittance_import_run) {
            window.open(`/app/ndis-crm-actual-remittance-import-run/${doc.ndis_actual_remittance_import_run}`, "_blank")
          } else {
            createToast({ title: "No Actual Remittance Import Run linked yet", icon: "info" })
          }
        }
      }
'''.rstrip()
...
def _prep_script():
	try:
		from ndis_crm.setup.phase24_remittance_import_preparation import _run_script as previous_script

		script = previous_script()
	except Exception:
		script = 'frappe.ui.form.on("NDIS CRM Remittance Import Preparation Run", { refresh(frm) { if (frm.is_new()) { return; } } });'
	insert = r'''

        frm.add_custom_button(__("Create Actual Remittance Import Run"), function () {
            frappe.call({
                method: "ndis_crm.phase25_actual_remittance_import.create_actual_remittance_import_run_from_preparation_run",
                args: { remittance_import_preparation_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Creating actual remittance import run...")
            }).then((r) => {
                if (r.message && r.message.name) {
                    frappe.show_alert({ message: r.message.message || __("Actual Remittance Import Run created"), indicator: "green" });
                    frm.reload_doc();
                    frappe.set_route("Form", "NDIS CRM Actual Remittance Import Run", r.message.name);
                }
            });
        }, __("Actions"));

        if (frm.doc.ndis_actual_remittance_import_run) {
            frm.add_custom_button(__("Open Actual Remittance Import Run"), function () {
                frappe.set_route("Form", "NDIS CRM Actual Remittance Import Run", frm.doc.ndis_actual_remittance_import_run);
            }, __("Open"));
        }
'''
	return script.replace("\n    }\n});", insert + "\n    }\n});", 1)
