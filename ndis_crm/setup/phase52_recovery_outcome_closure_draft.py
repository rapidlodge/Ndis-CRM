...
def install():
    ...
    create_recovery_outcome_closure_draft_doctypes()
    create_custom_fields_phase52()
    create_form_scripts()
    frappe.clear_cache()
    frappe.db.commit()
    print("NDIS CRM Phase 52 Recovery Outcome Closure Draft gate installed successfully.")
...
def repair_partial_crm_deal_phase52_fields():
    expected = {
    ...
    for fieldname, values in expected.items():
        ...
        if changed:
            ...
            print(f"Repaired CRM Deal Phase 52 field: {custom_field_name}")


def repair_partial_shared_phase52_fields(doctypes):
    expected = {
        "ndis_recovery_outcome_closure_draft_run": {
            "fieldtype": "Link",
            "options": RECOVERY_OUTCOME_CLOSURE_DRAFT_RUN,
        },
        "recovery_outcome_closure_draft_status": {
            "fieldtype": "Data",
            "options": None,
    ...
    for doctype in doctypes:
        for fieldname, values in expected.items():
            ...
            if changed:
                custom_field.save(ignore_permissions=True)
                print(f"Repaired shared Phase 52 field: {custom_field_name}")


def remove_partial_intake_phase52_fields():
    for fieldname in [
        "ndis_recovery_outcome_closure_draft_run",
        "recovery_outcome_closure_draft_status",
        "recovery_outcome_closure_draft_ready",
        "recovery_outcome_closure_draft_section",
    ]:
        ...
        print(f"Removed partial Intake Phase 52 field: {custom_field_name}")
...
def ensure_required_doctypes():
    ...
    if missing:
        frappe.throw("Missing required Phase 52 DocTypes: " + ", ".join(missing))

    print("Required Phase 52 DocTypes found.")
...
def create_recovery_outcome_closure_draft_doctypes():
    create_recovery_outcome_closure_draft_line()
    create_recovery_outcome_closure_draft_run()


def create_recovery_outcome_closure_draft_line():
    fields = [
        make_field("Source", "source_section", "Section Break"),
        make_field("Recovery Outcome Closure Draft Source Key", "recovery_outcome_closure_draft_source_key", "Data", read_only=1),

        make_field("Draft Target", "draft_target_section", "Section Break"),
        make_field("Target Closure DocType", "target_closure_doctype", "Data", read_only=1),
        make_field("Target Closure Name", "target_closure_name", "Data", read_only=1),
        make_field("Target Closure Status", "target_closure_status", "Data", read_only=1),
        ...
        make_field("Closure Draft", "closure_draft_section", "Section Break"),
        make_field("Recovery Outcome Closure Draft Source Ready", "recovery_outcome_closure_draft_source_ready", "Check", default="0"),
        ...
        make_field("Closure Draft Review Complete", "closure_draft_review_complete", "Check", default="0"),
        make_field("Closure Draft Authorized", "closure_draft_authorized", "Check", default="0"),
        make_field("Closure Draft Creation Authorized", "closure_draft_creation_authorized", "Check", default="0"),

        make_field("Blocked Downstream Controls", "blocked_downstream_section", "Section Break"),
        make_field("Actual Recovery Case Closure Authorized", "actual_recovery_case_closure_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Communication Creation Authorized", "communication_creation_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Email Send Authorized", "email_send_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Event Creation Authorized", "event_creation_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("ToDo Creation Authorized", "todo_creation_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Task Creation Authorized", "task_creation_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Recovery Case Creation Authorized", "recovery_case_creation_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Journal Entry Authorized", "journal_entry_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Manual GL Authorized", "manual_gl_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Payment Entry Authorized", "payment_entry_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Sales Invoice Authorized", "sales_invoice_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Adjustment Authorized", "adjustment_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Bank Reconciliation Authorized", "bank_reconciliation_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Claim Batch Authorized", "claim_batch_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Claim Line Authorized", "claim_line_authorized", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Remittance Import Authorized", "remittance_import_authorized", "Check", default="0", description="Blocked in Phase 52."),

        make_field("Line Status", "line_status_section", "Section Break"),
        make_field("Recovery Outcome Closure Draft Hold", "recovery_outcome_closure_draft_hold", "Check", default="1", in_list_view=1),
        make_field("Recovery Outcome Closure Draft Hold Reason", "recovery_outcome_closure_draft_hold_reason", "Small Text"),
        make_field("Line Ready for Recovery Outcome Closure Draft", "line_ready_for_recovery_outcome_closure_draft", "Check", default="0", in_list_view=1),
        make_field("Recovery Outcome Closure Draft Line Status", "recovery_outcome_closure_draft_line_status", "Select", options="Draft\nReady\nApproved\nCRM Closure Draft Prepared\nReturned\nCancelled", default="Draft", in_list_view=1),
        make_field("Notes", "notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=RECOVERY_OUTCOME_CLOSURE_DRAFT_LINE,
        fields=fields,
        istable=1,
    )


def create_recovery_outcome_closure_draft_run():
    fields = [
        make_field("Series", "naming_series", "Select", options="NDIS-RECOVERY-CLOSURE-DRAFT-.YYYY.-.#####", default="NDIS-RECOVERY-CLOSURE-DRAFT-.YYYY.-.#####", reqd=1),

        make_field("Status", "status_section", "Section Break"),
        make_field("Status", "status", "Select", options="Draft\nIn Review\nReady for Closure Draft Creation\nClosure Draft Creation Approved\nClosure Drafts Created\nReturned to Closure Preparation\nCancelled", default="Draft", in_list_view=1),
        make_field("Readiness %", "readiness_percent", "Percent", read_only=1, in_list_view=1),
        make_field("Recovery Outcome Closure Draft Run Ready", "recovery_outcome_closure_draft_run_ready", "Check", read_only=1, in_list_view=1),
        make_field("No Recovery Outcome Closure Draft Candidate Found", "no_recovery_outcome_closure_draft_candidate_found", "Check", default="0", read_only=1),

        make_field("Target Closure Draft Mode", "target_closure_draft_mode", "Select", options="CRM Closure Draft Only", default="CRM Closure Draft Only"),
        make_field("Closure Draft Creation Allowed", "closure_draft_creation_allowed", "Check", default="0"),

        make_field("Actual Recovery Case Closure Allowed", "actual_recovery_case_closure_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Communication Creation Allowed", "communication_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Email Send Allowed", "email_send_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Event Creation Allowed", "event_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("ToDo Creation Allowed", "todo_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Task Creation Allowed", "task_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Recovery Case Creation Allowed", "recovery_case_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Journal Entry Creation Allowed", "journal_entry_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Manual GL Creation Allowed", "manual_gl_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Payment Entry Creation Allowed", "payment_entry_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Sales Invoice Creation Allowed", "sales_invoice_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Adjustment Creation Allowed", "adjustment_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Bank Reconciliation Allowed", "bank_reconciliation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Claim Batch Creation Allowed", "claim_batch_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Claim Line Creation Allowed", "claim_line_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),
        make_field("Remittance Import Creation Allowed", "remittance_import_creation_allowed", "Check", default="0", description="Blocked in Phase 52."),

        make_field("Summary", "summary_section", "Section Break"),
        make_field("Recovery Outcome Closure Draft Line Count", "recovery_outcome_closure_draft_line_count", "Int", read_only=1),
        make_field("Recovery Outcome Closure Draft Amount Total", "recovery_outcome_closure_draft_amount_total", "Currency", read_only=1),
        make_field("Recovery Outcome Closure Draft Ready Count", "recovery_outcome_closure_draft_ready_count", "Int", read_only=1),
        make_field("Recovery Outcome Closure Draft Hold Count", "recovery_outcome_closure_draft_hold_count", "Int", read_only=1),
        make_field("CRM Closure Draft Count", "crm_closure_draft_count", "Int", read_only=1),
        make_field("Closure Draft Created Count", "closure_draft_created_count", "Int", read_only=1),
    ...
    fields += [
        ...
        make_field("NDIS CRM Recovery Outcome Closure Preparation Run", "recovery_outcome_closure_preparation_run", "Link", options=RECOVERY_OUTCOME_CLOSURE_PREPARATION_RUN, reqd=1, in_list_view=1),
        ...
        make_field("Recovery Outcome Closure Draft Owner", "recovery_outcome_closure_draft_owner", "Link", options="User", in_list_view=1),
        make_field("Recovery Outcome Closure Preparation Owner", "recovery_outcome_closure_preparation_owner", "Link", options="User"),
        make_field("Created By", "created_by", "Link", options="User", read_only=1),
        make_field("Created On", "created_on", "Datetime", read_only=1),

        make_field("Recovery Outcome Closure Draft Lines", "recovery_outcome_closure_draft_lines_section", "Section Break"),
        make_field("Recovery Outcome Closure Draft Lines", "recovery_outcome_closure_draft_lines", "Table", options=RECOVERY_OUTCOME_CLOSURE_DRAFT_LINE),

        make_field("Notes", "notes_section", "Section Break"),
        make_field("Recovery Outcome Closure Draft Notes", "recovery_outcome_closure_draft_notes", "Small Text"),
        make_field("Returned / Blocker Notes", "returned_notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=RECOVERY_OUTCOME_CLOSURE_DRAFT_RUN,
        fields=fields,
...
def create_custom_fields_phase52():
    deal_fields = [
        {
            "fieldname": "recovery_outcome_closure_draft_section",
            "label": "NDIS Recovery Outcome Closure Draft Run",
            "fieldtype": "Section Break",
            "insert_after": "recovery_outcome_closure_preparation_ready",
        },
        {
            "fieldname": "ndis_recovery_outcome_closure_draft_run_required",
            "label": "Recovery Outcome Closure Draft Required Before Active Deal",
            "fieldtype": "Check",
            "default": "0",
            "insert_after": "recovery_outcome_closure_draft_section",
        },
        {
            "fieldname": "ndis_recovery_outcome_closure_draft_run",
            "label": "NDIS CRM Recovery Outcome Closure Draft Run",
            "fieldtype": "Small Text",
            "read_only": 1,
            "insert_after": "ndis_recovery_outcome_closure_draft_run_required",
        },
        {
            "fieldname": "recovery_outcome_closure_draft_status",
            "label": "Recovery Outcome Closure Draft Status",
            "fieldtype": "Small Text",
            "read_only": 1,
            "insert_after": "ndis_recovery_outcome_closure_draft_run",
        },
        {
            "fieldname": "recovery_outcome_closure_draft_ready",
            "label": "Recovery Outcome Closure Draft Ready",
            "fieldtype": "Check",
            "read_only": 1,
            "insert_after": "recovery_outcome_closure_draft_status",
        },
    ]
    ...
    repair_partial_crm_deal_phase52_fields()
    repair_partial_shared_phase52_fields(shared_doctypes)
    remove_partial_intake_phase52_fields()
    safe_create_custom_fields(custom_fields)
    print("Created / updated Phase 52 recovery outcome closure draft chain custom fields.")
...
def _phase52_deal_actions():
    return r'''
// NDIS CRM Phase 52 Deal Actions
{
  label: "Create Recovery Outcome Closure Draft Run",
  onClick: () => {
    call("ndis_crm.phase52_recovery_outcome_closure_draft.create_recovery_outcome_closure_draft_run_from_crm_deal", {
      deal: doc.name
    }).then((data) => {
      if (data && data.name) {
        createToast({
          title: data.created ? "Recovery Outcome Closure Draft Run Created" : "Existing Recovery Outcome Closure Draft Run Found",
          icon: "check",
          iconClasses: "text-green-600",
        })
        window.open(`/app/ndis-crm-recovery-outcome-closure-draft-run/${data.name}`, "_blank")
      }
    })
  }
},
{
  label: "Open Recovery Outcome Closure Draft Run",
  onClick: () => {
    if (doc.ndis_recovery_outcome_closure_draft_run) {
      window.open(`/app/ndis-crm-recovery-outcome-closure-draft-run/${doc.ndis_recovery_outcome_closure_draft_run}`, "_blank")
    } else {
      createToast({ title: "No Recovery Outcome Closure Draft Run linked yet", icon: "info" })
    }
  }
}
'''.strip()
...
def _deal_script():
    try:
        from ndis_crm.setup.phase51_recovery_outcome_closure_preparation import _deal_script as phase51_deal_script

        script = phase51_deal_script()
    except Exception:
        script = '''
...
'''.strip()
    ...
    if "NDIS CRM Phase 52 Deal Actions" in script:
        return script

    return _append_before_last(script, "\n    ]", _phase52_deal_actions())
...
frappe.ui.form.on("NDIS CRM Recovery Outcome Closure Preparation Run", {
    refresh(frm) {
        ...
        frm.add_custom_button(__("Create Recovery Outcome Closure Draft Run"), function () {
            frappe.call({
                method: "ndis_crm.phase52_recovery_outcome_closure_draft.create_recovery_outcome_closure_draft_run_from_closure_preparation_run",
                args: { recovery_outcome_closure_preparation_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Creating recovery outcome closure draft run...")
            }).then((r) => {
                if (r.message && r.message.name) {
                    frappe.show_alert({ message: r.message.message || __("Recovery Outcome Closure Draft Run created"), indicator: "green" });
                    frm.reload_doc();
                    frappe.set_route("Form", "NDIS CRM Recovery Outcome Closure Draft Run", r.message.name);
                }
            });
        }, __("Actions"));

        if (frm.doc.ndis_recovery_outcome_closure_draft_run) {
            frm.add_custom_button(__("Open Recovery Outcome Closure Draft Run"), function () {
                frappe.set_route("Form", "NDIS CRM Recovery Outcome Closure Draft Run", frm.doc.ndis_recovery_outcome_closure_draft_run);
            }, __("Open"));
        }
    }
});
...
frappe.ui.form.on("NDIS CRM Recovery Outcome Closure Draft Run", {
    refresh(frm) {
        ...
        frm.add_custom_button(__("Generate Draft Lines"), function () {
            frappe.call({
                method: "ndis_crm.phase52_recovery_outcome_closure_draft.generate_recovery_outcome_closure_draft_lines",
                args: { recovery_outcome_closure_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Generating closure draft lines...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Closure draft lines generated"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));
        ...
        frm.add_custom_button(__("Validate Draft Readiness"), function () {
            frappe.call({
                method: "ndis_crm.phase52_recovery_outcome_closure_draft.validate_recovery_outcome_closure_draft_readiness",
                args: { recovery_outcome_closure_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Validating closure draft readiness...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Closure draft readiness validated"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));
        ...
        frm.add_custom_button(__("Mark Ready for Draft Creation"), function () {
            frappe.call({
                method: "ndis_crm.phase52_recovery_outcome_closure_draft.mark_ready_for_recovery_outcome_closure_draft",
                args: { recovery_outcome_closure_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Marking ready for closure draft creation...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Ready for closure draft creation"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));

        frm.add_custom_button(__("Approve Draft Run"), function () {
            frappe.call({
                method: "ndis_crm.phase52_recovery_outcome_closure_draft.approve_recovery_outcome_closure_draft_run",
                args: { recovery_outcome_closure_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Approving closure draft run...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Closure Draft Run approved"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));

        frm.add_custom_button(__("Create Closure Drafts"), function () {
            frappe.confirm(
                __("This creates CRM closure draft records only. It will not close recovery cases, create communications, tasks, emails, accounting, claim, remittance, adjustment, or bank reconciliation records."),
                function () {
                    frappe.call({
                        method: "ndis_crm.phase52_recovery_outcome_closure_draft.create_recovery_outcome_closure_drafts",
                        args: { recovery_outcome_closure_draft_run: frm.doc.name },
                        freeze: true,
                        freeze_message: __("Creating closure drafts...")
                    }).then((r) => {
                        if (r.message) {
                            frappe.show_alert({ message: r.message.message || __("Closure drafts created"), indicator: "green" });
                            frm.reload_doc();
                        }
                    });
                }
        ...
        if (frm.doc.recovery_outcome_closure_preparation_run) {
            frm.add_custom_button(__("Open Closure Preparation Run"), function () {
                frappe.set_route("Form", "NDIS CRM Recovery Outcome Closure Preparation Run", frm.doc.recovery_outcome_closure_preparation_run);
            }, __("Open"));
        }
