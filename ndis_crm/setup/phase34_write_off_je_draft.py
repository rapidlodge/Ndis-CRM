...
def install():
    ...
    create_write_off_je_draft_doctypes()
    create_custom_fields_phase34()
    create_journal_entry_custom_fields()
    create_form_scripts()
    frappe.clear_cache()
    frappe.db.commit()
    print("NDIS CRM Phase 34 Write Off Journal Entry Draft gate installed successfully.")
...
def ensure_required_doctypes():
    ...
    if missing:
        frappe.throw("Missing required Phase 34 DocTypes: " + ", ".join(missing))

    print("Required Phase 34 DocTypes found.")
...
def create_write_off_je_draft_doctypes():
    create_write_off_je_draft_line()
    create_write_off_je_draft_run()


def create_write_off_je_draft_line():
    fields = [
        make_field("Source", "source_section", "Section Break"),
        make_field("Write Off JE Draft Source Key", "write_off_je_draft_source_key", "Data", read_only=1),
        ...
        make_field("Journal Entry Draft", "journal_entry_section", "Section Break"),
        ...
        make_field("JE Draft Values", "je_draft_values_section", "Section Break"),
        ...
        make_field("Journal Entry Posting Date", "journal_entry_posting_date", "Date"),
        make_field("Write Off Expense Account", "write_off_expense_account", "Link", options="Account", description="Debit account for write-off expense/loss."),
        make_field("Receivable Account", "receivable_account", "Link", options="Account", description="Credit account, normally Accounts Receivable."),
        make_field("Party Type", "party_type", "Data", default="Customer"),
        ...
        make_field("Write Off JE Draft Source Ready", "write_off_je_draft_source_ready", "Check", default="0"),
        make_field("Journal Entry Account Review Complete", "journal_entry_account_review_complete", "Check", default="0"),
        make_field("Journal Entry Draft Authorized", "journal_entry_draft_authorized", "Check", default="0"),
        make_field("Journal Entry Submit Authorized", "journal_entry_submit_authorized", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Manual GL Authorized", "manual_gl_authorized", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Payment Entry Authorized", "payment_entry_authorized", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Write Off Submit Authorized", "write_off_submit_authorized", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Recovery Authorized", "recovery_authorized", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Adjustment Authorized", "adjustment_authorized", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Bank Reconciliation Authorized", "bank_reconciliation_authorized", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Write Off JE Draft Hold", "write_off_je_draft_hold", "Check", default="1", in_list_view=1),
        make_field("Write Off JE Draft Hold Reason", "write_off_je_draft_hold_reason", "Small Text"),

        make_field("Line Status", "line_status_section", "Section Break"),
        make_field("Line Ready for Journal Entry Draft", "line_ready_for_journal_entry_draft", "Check", default="0", in_list_view=1),
        make_field("Write Off JE Draft Line Status", "write_off_je_draft_line_status", "Select", options="Draft\nReady\nApproved\nCRM JE Draft Prepared\nDraft Journal Entry Created\nReturned\nCancelled", default="Draft", in_list_view=1),
        make_field("Notes", "notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=WRITE_OFF_JE_DRAFT_LINE,
        fields=fields,
        istable=1,
    )


def create_write_off_je_draft_run():
    fields = [
        make_field("Series", "naming_series", "Select", options="NDIS-WRITEOFF-JE-DRAFT-.YYYY.-.#####", default="NDIS-WRITEOFF-JE-DRAFT-.YYYY.-.#####", reqd=1),

        make_field("Status", "status_section", "Section Break"),
        make_field("Status", "status", "Select", options="Draft\nIn Review\nReady for Journal Entry Draft Creation\nJournal Entry Draft Run Approved\nJournal Entry Drafts Created\nReturned to Write Off Draft\nCancelled", default="Draft", in_list_view=1),
        make_field("Readiness %", "readiness_percent", "Percent", read_only=1, in_list_view=1),
        make_field("Write Off JE Draft Run Ready", "write_off_je_draft_run_ready", "Check", read_only=1, in_list_view=1),
        make_field("No Write Off JE Draft Candidate Found", "no_write_off_je_draft_candidate_found", "Check", default="0", read_only=1),
        make_field("Target Journal Mode", "target_journal_mode", "Select", options="CRM JE Draft Preparation Only\nDraft Journal Entry If Accounts Ready", default="CRM JE Draft Preparation Only"),
        make_field("Journal Entry Draft Creation Allowed", "journal_entry_draft_creation_allowed", "Check", default="0"),
        make_field("ERPNext Journal Entry Draft Creation Allowed", "erpnext_journal_entry_draft_creation_allowed", "Check", default="0", description="Only used with Draft Journal Entry If Accounts Ready mode."),
        make_field("Journal Entry Submit Allowed", "journal_entry_submit_allowed", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Manual GL Creation Allowed", "manual_gl_creation_allowed", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Payment Entry Creation Allowed", "payment_entry_creation_allowed", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Write Off Submit Allowed", "write_off_submit_allowed", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Recovery Creation Allowed", "recovery_creation_allowed", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Adjustment Creation Allowed", "adjustment_creation_allowed", "Check", default="0", description="Blocked in Phase 34."),
        make_field("Bank Reconciliation Allowed", "bank_reconciliation_allowed", "Check", default="0", description="Blocked in Phase 34."),

        make_field("Summary", "summary_section", "Section Break"),
        make_field("Write Off JE Draft Line Count", "write_off_je_draft_line_count", "Int", read_only=1),
        make_field("Write Off JE Draft Amount Total", "write_off_je_draft_amount_total", "Currency", read_only=1),
        make_field("Write Off JE Draft Ready Count", "write_off_je_draft_ready_count", "Int", read_only=1),
        make_field("Write Off JE Draft Hold Count", "write_off_je_draft_hold_count", "Int", read_only=1),
        make_field("CRM JE Draft Preparation Count", "crm_je_draft_preparation_count", "Int", read_only=1),
        make_field("Journal Entry Draft Count", "journal_entry_draft_count", "Int", read_only=1),
        make_field("Blocked Journal Entry Submit Count", "blocked_journal_entry_submit_count", "Int", read_only=1),
    ...
    fields += [
        ...
        make_field("NDIS CRM Write Off Draft Run", "write_off_draft_run", "Link", options=WRITE_OFF_DRAFT_RUN, reqd=1, in_list_view=1),
    ...
    fields += [
        ...
        make_field("Write Off JE Draft Owner", "write_off_je_draft_owner", "Link", options="User", in_list_view=1),
        make_field("Write Off Draft Owner", "write_off_draft_owner", "Link", options="User"),
        make_field("Drafted By", "drafted_by", "Link", options="User", read_only=1),
        make_field("Drafted On", "drafted_on", "Datetime", read_only=1),

        make_field("Write Off JE Draft Lines", "write_off_je_draft_lines_section", "Section Break"),
        make_field("Write Off JE Draft Lines", "write_off_je_draft_lines", "Table", options=WRITE_OFF_JE_DRAFT_LINE),

        make_field("Notes", "notes_section", "Section Break"),
        make_field("Write Off JE Draft Notes", "write_off_je_draft_notes", "Small Text"),
        make_field("Returned / Blocker Notes", "returned_notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=WRITE_OFF_JE_DRAFT_RUN,
        fields=fields,
...
def create_custom_fields_phase34():
    deal_fields = [
        {
            "fieldname": "write_off_je_draft_section",
            "label": "NDIS Write Off JE Draft Run",
            "fieldtype": "Section Break",
            "insert_after": "write_off_draft_ready",
        },
        {
            "fieldname": "ndis_write_off_je_draft_run_required",
            "label": "Write Off JE Draft Required Before Active Deal",
            "fieldtype": "Check",
            "default": "0",
            "insert_after": "write_off_je_draft_section",
            "description": "Optional guard only. Write-off JE draft normally happens after write-off draft creation.",
        },
        {
            "fieldname": "ndis_write_off_je_draft_run",
            "label": "NDIS CRM Write Off JE Draft Run",
            "fieldtype": "Link",
            "options": WRITE_OFF_JE_DRAFT_RUN,
            "read_only": 1,
            "insert_after": "ndis_write_off_je_draft_run_required",
        },
        {
            "fieldname": "write_off_je_draft_status",
            "label": "Write Off JE Draft Status",
            "fieldtype": "Data",
            "read_only": 1,
            "insert_after": "ndis_write_off_je_draft_run",
        },
        {
            "fieldname": "write_off_je_draft_ready",
            "label": "Write Off JE Draft Ready",
            "fieldtype": "Check",
            "read_only": 1,
            "insert_after": "write_off_je_draft_status",
        },
    ]
    ...
    print("Created / updated Phase 34 write-off JE draft chain custom fields.")
...
def create_journal_entry_custom_fields():
    custom_fields = {
        "Journal Entry": [
            {
                "fieldname": "ndis_crm_write_off_je_draft_section",
                "label": "NDIS CRM Write Off JE Draft Control",
                "fieldtype": "Section Break",
                "insert_after": "user_remark",
            },
            {
                "fieldname": "ndis_crm_write_off_je_draft_run",
                "label": "NDIS CRM Write Off JE Draft Run",
                "fieldtype": "Link",
                "options": WRITE_OFF_JE_DRAFT_RUN,
                "read_only": 1,
                "insert_after": "ndis_crm_write_off_je_draft_section",
            },
            {
                "fieldname": "ndis_crm_write_off_je_draft_line",
                "label": "NDIS CRM Write Off JE Draft Line",
                "fieldtype": "Data",
                "read_only": 1,
                "insert_after": "ndis_crm_write_off_je_draft_run",
            },
            {
                ...
                "fieldtype": "Check",
                "default": "1",
                "insert_after": "ndis_crm_write_off_je_draft_line",
            },
        ]
    }

    create_custom_fields(custom_fields, update=True)
    print("Created / updated Phase 34 Journal Entry draft guard custom fields.")
...
def _phase34_deal_actions():
    return r'''
// NDIS CRM Phase 34 Deal Actions
{
  label: "Create Write Off JE Draft Run",
  onClick: () => {
    call("ndis_crm.phase34_write_off_je_draft.create_write_off_je_draft_run_from_crm_deal", {
      deal: doc.name
    }).then((data) => {
      if (data && data.name) {
        createToast({
          title: data.created ? "Write Off JE Draft Run Created" : "Existing Write Off JE Draft Run Found",
          icon: "check",
          iconClasses: "text-green-600",
        })
        window.open(`/app/ndis-crm-write-off-je-draft-run/${data.name}`, "_blank")
      }
    })
  }
},
{
  label: "Open Write Off JE Draft Run",
  onClick: () => {
    if (doc.ndis_write_off_je_draft_run) {
      window.open(`/app/ndis-crm-write-off-je-draft-run/${doc.ndis_write_off_je_draft_run}`, "_blank")
    } else {
      createToast({ title: "No Write Off JE Draft Run linked yet", icon: "info" })
    }
  }
}
'''.strip()
...
def _deal_script():
    try:
        from ndis_crm.setup.phase33_write_off_draft import _deal_script as phase33_deal_script
        script = phase33_deal_script()
    except Exception:
        script = '''
...
'''.strip()
    ...
    if "NDIS CRM Phase 34 Deal Actions" in script:
        return script

    return _append_before_last(script, "\n    ]", _phase34_deal_actions())
...
frappe.ui.form.on("NDIS CRM Write Off Draft Run", {
    refresh(frm) {
        ...
        frm.add_custom_button(__("Create Write Off JE Draft Run"), function () {
            frappe.call({
                method: "ndis_crm.phase34_write_off_je_draft.create_write_off_je_draft_run_from_write_off_draft_run",
                args: { write_off_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Creating write-off JE draft run...")
            }).then((r) => {
                if (r.message && r.message.name) {
                    frappe.show_alert({ message: r.message.message || __("Write Off JE Draft Run created"), indicator: "green" });
                    frm.reload_doc();
                    frappe.set_route("Form", "NDIS CRM Write Off JE Draft Run", r.message.name);
                }
            });
        }, __("Actions"));

        if (frm.doc.ndis_write_off_je_draft_run) {
            frm.add_custom_button(__("Open Write Off JE Draft Run"), function () {
                frappe.set_route("Form", "NDIS CRM Write Off JE Draft Run", frm.doc.ndis_write_off_je_draft_run);
            }, __("Open"));
        }
    }
});
