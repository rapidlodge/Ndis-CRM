...
def install():
    ...
    create_payment_entry_draft_doctypes()
    create_custom_fields_phase28()
    create_payment_entry_custom_fields()
    create_form_scripts()
    frappe.clear_cache()
    frappe.db.commit()
    print("NDIS CRM Phase 28 Payment Entry Draft bridge installed successfully.")
...
def ensure_required_doctypes():
    ...
    if missing:
        frappe.throw("Missing required Phase 28 DocTypes: " + ", ".join(missing))

    print("Required Phase 28 DocTypes found.")
...
def create_payment_entry_draft_doctypes():
    create_payment_entry_draft_line()
    create_payment_entry_draft_run()


def create_payment_entry_draft_line():
    fields = [
        make_field("Source", "source_section", "Section Break"),
        make_field("Payment Entry Source Key", "payment_entry_source_key", "Data", read_only=1),
        make_field("Payment Entry", "payment_entry", "Link", options="Payment Entry", read_only=1, in_list_view=1),
        make_field("Payment Entry Docstatus", "payment_entry_docstatus", "Int", read_only=1),

        make_field("Remittance / Claim Links", "remittance_claim_links_section", "Section Break"),
    ...
    fields += [
        ...
        make_field("Payment Reference", "payment_reference_section", "Section Break"),
        ...
        make_field("Payment Entry Mapping", "payment_entry_mapping_section", "Section Break"),
        make_field("Payment Entry Type", "payment_entry_type", "Select", options="Receive", default="Receive"),
        make_field("Party Type", "party_type", "Select", options="Customer", default="Customer"),
        make_field("Party", "party", "Link", options="Customer"),
        make_field("Receivable Account", "receivable_account", "Link", options="Account"),
        make_field("Paid To Account", "paid_to_account", "Link", options="Account"),
        make_field("Mode of Payment", "mode_of_payment", "Link", options="Mode of Payment"),
        make_field("Payment Entry Posting Date", "payment_entry_posting_date", "Date"),
        make_field("Payment Entry Reference No", "payment_entry_reference_no", "Data"),
        make_field("Payment Entry Reference Date", "payment_entry_reference_date", "Date"),

        make_field("Draft Controls", "draft_controls_section", "Section Break"),
        make_field("Payment Entry Source Ready", "payment_entry_source_ready", "Check", default=0),
        make_field("Payment Entry Review Complete", "payment_entry_review_complete", "Check", default=0),
        make_field("Payment Entry Draft Authorized", "payment_entry_draft_authorized", "Check", default=0),
        make_field("Payment Entry Submit Authorized", "payment_entry_submit_authorized", "Check", default=0, description="Blocked in Phase 28."),
        make_field("Journal Authorized", "journal_authorized", "Check", default=0, description="Blocked in Phase 28."),
        make_field("Write Off Authorized", "write_off_authorized", "Check", default=0, description="Blocked in Phase 28."),
        make_field("Recovery Authorized", "recovery_authorized", "Check", default=0, description="Blocked in Phase 28."),
        make_field("Bank Reconciliation Authorized", "bank_reconciliation_authorized", "Check", default=0, description="Blocked in Phase 28."),
        make_field("Payment Entry Hold", "payment_entry_hold", "Check", default=1, in_list_view=1),
        make_field("Payment Entry Hold Reason", "payment_entry_hold_reason", "Small Text"),

        make_field("Line Status", "line_status_section", "Section Break"),
        make_field("Line Ready for Payment Entry Draft", "line_ready_for_payment_entry_draft", "Check", default=0, in_list_view=1),
        make_field("Payment Entry Line Status", "payment_entry_line_status", "Select", options="Draft\nReady\nApproved\nDraft Payment Entry Created\nExcluded / No Payment\nReturned\nCancelled", default="Draft", in_list_view=1),
        make_field("Notes", "notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=PAYMENT_ENTRY_DRAFT_LINE,
        fields=fields,
        istable=1,
    )


def create_payment_entry_draft_run():
    fields = [
        make_field("Series", "naming_series", "Select", options="NDIS-PAY-ENTRY-DRAFT-.YYYY.-.#####", default="NDIS-PAY-ENTRY-DRAFT-.YYYY.-.#####", reqd=1),

        make_field("Status", "status_section", "Section Break"),
        make_field("Status", "status", "Select", options="Draft\nIn Review\nReady for Payment Entry Draft Creation\nPayment Entry Draft Run Approved\nDraft Payment Entries Created\nReturned to Payment Allocation Preparation\nCancelled", default="Draft", in_list_view=1),
        make_field("Readiness %", "readiness_percent", "Percent", read_only=1, in_list_view=1),
        make_field("Payment Entry Draft Run Ready", "payment_entry_draft_run_ready", "Check", read_only=1, in_list_view=1),
        make_field("Payment Entry Draft Creation Allowed", "payment_entry_draft_creation_allowed", "Check", default=0),
        make_field("Payment Entry Submission Allowed", "payment_entry_submission_allowed", "Check", default=0, description="Blocked in Phase 28."),
        make_field("Journal Creation Allowed", "journal_creation_allowed", "Check", default=0, description="Blocked in Phase 28."),
        make_field("Write Off Creation Allowed", "write_off_creation_allowed", "Check", default=0, description="Blocked in Phase 28."),
        make_field("Recovery Creation Allowed", "recovery_creation_allowed", "Check", default=0, description="Blocked in Phase 28."),
        make_field("Bank Reconciliation Allowed", "bank_reconciliation_allowed", "Check", default=0, description="Blocked in Phase 28."),

        make_field("Payment Entry Defaults", "payment_entry_defaults_section", "Section Break"),
        make_field("Receivable Account", "receivable_account", "Link", options="Account"),
        make_field("Paid To Account", "paid_to_account", "Link", options="Account"),
        make_field("Mode of Payment", "mode_of_payment", "Link", options="Mode of Payment"),
        make_field("Payment Entry Posting Date", "payment_entry_posting_date", "Date"),
        ...
        make_field("Actual Payment Reference", "actual_payment_reference", "Data", read_only=1),

        make_field("Summary", "summary_section", "Section Break"),
        make_field("Payment Entry Line Count", "payment_entry_line_count", "Int", read_only=1),
        make_field("Active Payment Line Count", "active_payment_line_count", "Int", read_only=1),
        make_field("Proposed Payment Amount Total", "proposed_payment_amount_total", "Currency", read_only=1),
        make_field("Draft Payment Entry Count", "draft_payment_entry_count", "Int", read_only=1),
        make_field("Payment Entry Ready Count", "payment_entry_ready_count", "Int", read_only=1),
        make_field("Payment Entry Hold Count", "payment_entry_hold_count", "Int", read_only=1),
        make_field("Excluded No Payment Count", "excluded_no_payment_count", "Int", read_only=1),
    ...
    fields += [
        ...
        make_field("NDIS CRM Payment Allocation Preparation Run", "payment_allocation_preparation_run", "Link", options=PAYMENT_ALLOCATION_PREP_RUN, reqd=1, in_list_view=1),
    ...
    fields += [
        ...
        make_field("Payment Entry Draft Owner", "payment_entry_draft_owner", "Link", options="User", in_list_view=1),
        make_field("Payment Allocation Owner", "payment_allocation_owner", "Link", options="User"),

        make_field("Payment Entry Lines", "payment_entry_lines_section", "Section Break"),
        make_field("Payment Entry Lines", "payment_entry_lines", "Table", options=PAYMENT_ENTRY_DRAFT_LINE),

        make_field("Notes", "notes_section", "Section Break"),
        make_field("Payment Entry Draft Notes", "payment_entry_draft_notes", "Small Text"),
        make_field("Returned / Blocker Notes", "returned_notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=PAYMENT_ENTRY_DRAFT_RUN,
        fields=fields,
...
def create_custom_fields_phase28():
    deal_fields = [
        {
            "fieldname": "payment_entry_draft_section",
            "label": "NDIS Payment Entry Draft Run",
            "fieldtype": "Section Break",
            "insert_after": "payment_allocation_preparation_ready",
        },
        {
            "fieldname": "ndis_payment_entry_draft_run_required",
            "label": "Payment Entry Draft Required Before Active Deal",
            "fieldtype": "Check",
            "default": "0",
            "insert_after": "payment_entry_draft_section",
            "description": "Optional guard only. Payment Entry draft creation normally happens after payment allocation preparation.",
        },
        {
            "fieldname": "ndis_payment_entry_draft_run",
            "label": "NDIS CRM Payment Entry Draft Run",
            "fieldtype": "Link",
            "options": PAYMENT_ENTRY_DRAFT_RUN,
            "read_only": 1,
            "insert_after": "ndis_payment_entry_draft_run_required",
        },
        {
            "fieldname": "payment_entry_draft_status",
            "label": "Payment Entry Draft Status",
            "fieldtype": "Data",
            "read_only": 1,
            "insert_after": "ndis_payment_entry_draft_run",
        },
        {
            "fieldname": "payment_entry_draft_ready",
            "label": "Payment Entry Draft Ready",
            "fieldtype": "Check",
            "read_only": 1,
            "insert_after": "payment_entry_draft_status",
        },
    ]
    ...
    print("Created / updated Phase 28 Payment Entry draft chain custom fields.")
...
def create_payment_entry_custom_fields():
    custom_fields = {
        "Payment Entry": [
            {
                "fieldname": "ndis_crm_payment_entry_section",
                "label": "NDIS CRM Payment Entry Control",
                "fieldtype": "Section Break",
                "insert_after": "remarks",
            },
            {
                "fieldname": "ndis_crm_payment_entry_draft_run",
                "label": "NDIS CRM Payment Entry Draft Run",
                "fieldtype": "Link",
                "options": PAYMENT_ENTRY_DRAFT_RUN,
                "read_only": 1,
                "insert_after": "ndis_crm_payment_entry_section",
            },
            {
                "fieldname": "ndis_crm_payment_entry_source",
                "label": "NDIS CRM Payment Entry Source",
                "fieldtype": "Check",
                "default": "0",
                "read_only": 1,
                "insert_after": "ndis_crm_payment_entry_draft_run",
            },
            {
                "fieldname": "ndis_crm_payment_entry_submission_blocked",
                "label": "NDIS CRM Payment Entry Submission Blocked",
                "fieldtype": "Check",
                "default": "1",
                "insert_after": "ndis_crm_payment_entry_source",
            },
            {
                "fieldname": "ndis_crm_payment_entry_status",
                "label": "NDIS CRM Payment Entry Status",
                "fieldtype": "Data",
                "read_only": 1,
                "insert_after": "ndis_crm_payment_entry_submission_blocked",
            },
            {
                "fieldname": "ndis_remittance_import",
                "label": "NDIS Remittance Import",
                "fieldtype": "Link",
                "options": "NDIS Remittance Import",
                "read_only": 1,
            ...
            },
            {
                ...
                "options": "CRM Deal",
                "read_only": 1,
                "insert_after": "ndis_remittance_import",
            },
        ],
        "Payment Entry Reference": [
            {
                "fieldname": "ndis_crm_payment_entry_reference_section",
                "label": "NDIS CRM Reference Control",
                "fieldtype": "Section Break",
                "insert_after": "allocated_amount",
            },
            {
                "fieldname": "ndis_crm_payment_entry_draft_line",
                "label": "NDIS CRM Payment Entry Draft Line",
                "fieldtype": "Data",
                "read_only": 1,
                "insert_after": "ndis_crm_payment_entry_reference_section",
            },
            {
                "fieldname": "ndis_remittance_import",
                "label": "NDIS Remittance Import",
                ...
                "options": "NDIS Remittance Import",
                "read_only": 1,
                "insert_after": "ndis_crm_payment_entry_draft_line",
            },
            {
                "fieldname": "ndis_claim_batch",
                "label": "NDIS Claim Batch",
                "fieldtype": "Link",
                "options": "NDIS Claim Batch",
                "read_only": 1,
                "insert_after": "ndis_remittance_import",
            },
            {
                "fieldname": "ndis_claim_line",
                "label": "NDIS Claim Line",
                "fieldtype": "Link",
                "options": "NDIS Claim Line",
                "read_only": 1,
                "insert_after": "ndis_claim_batch",
            },
            {
                "fieldname": "ndis_service_line",
                "label": "NDIS Service Line",
                "fieldtype": "Link",
                "options": "NDIS Service Line",
                "read_only": 1,
                "insert_after": "ndis_claim_line",
            },
            {
                "fieldname": "actual_payment_reference",
                "label": "Actual Payment Reference",
                "fieldtype": "Data",
                "read_only": 1,
                "insert_after": "ndis_service_line",
            },
        ],
    }

    create_custom_fields(custom_fields, update=True)
    print("Created / updated Phase 28 Payment Entry control custom fields.")
...
def _append_before_last(script, anchor, insert):
    if anchor not in script:
        frappe.throw(f"Could not append Phase 28 action script because the anchor was not found: {anchor}")
...
def _deal_script():
    try:
        from ndis_crm.setup.phase27_payment_allocation_preparation import _deal_script as previous_script
...
,
      {
        label: "Create Payment Entry Draft Run",
        onClick: () => {
          call("ndis_crm.phase28_payment_entry_draft.create_payment_entry_draft_run_from_crm_deal", {
            deal: doc.name
          }).then((data) => {
            if (data && data.name) {
              createToast({
                title: data.created ? "Payment Entry Draft Run Created" : "Existing Payment Entry Draft Run Found",
                icon: "check",
                iconClasses: "text-green-600",
              })
              window.open(`/app/ndis-crm-payment-entry-draft-run/${data.name}`, "_blank")
            }
          })
        }
      },
      {
        label: "Open Payment Entry Draft Run",
        onClick: () => {
          if (doc.ndis_payment_entry_draft_run) {
            window.open(`/app/ndis-crm-payment-entry-draft-run/${doc.ndis_payment_entry_draft_run}`, "_blank")
          } else {
            createToast({ title: "No Payment Entry Draft Run linked yet", icon: "info" })
          }
        }
      }
'''.rstrip()
...
def create_form_scripts():
    ...
    if frappe.db.exists("DocType", "Client Script"):
        upsert_doc(
            ...
            {
                "dt": PAYMENT_ALLOCATION_PREP_RUN,
                "view": "Form",
                "enabled": 1,
                "script": _payment_allocation_script(),
            },
        )
...
def _payment_allocation_script():
    try:
        from ndis_crm.setup.phase27_payment_allocation_preparation import _run_script as previous_script

        script = previous_script()
    except Exception:
        script = 'frappe.ui.form.on("NDIS CRM Payment Allocation Preparation Run", { refresh(frm) { if (frm.is_new()) { return; } } });'

    insert = r'''

        frm.add_custom_button(__("Create Payment Entry Draft Run"), function () {
            frappe.call({
                method: "ndis_crm.phase28_payment_entry_draft.create_payment_entry_draft_run_from_payment_allocation_preparation_run",
                args: { payment_allocation_preparation_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Creating Payment Entry Draft Run...")
            }).then((r) => {
                if (r.message && r.message.name) {
                    frappe.show_alert({ message: r.message.message || __("Payment Entry Draft Run created"), indicator: "green" });
                    frm.reload_doc();
                    frappe.set_route("Form", "NDIS CRM Payment Entry Draft Run", r.message.name);
                }
            });
        }, __("Actions"));

        if (frm.doc.ndis_payment_entry_draft_run) {
            frm.add_custom_button(__("Open Payment Entry Draft Run"), function () {
                frappe.set_route("Form", "NDIS CRM Payment Entry Draft Run", frm.doc.ndis_payment_entry_draft_run);
            }, __("Open"));
        }
'''
    return _append_before_last(script, "\n    }\n});", insert)
...
def _run_script():
    return r'''
frappe.ui.form.on("NDIS CRM Payment Entry Draft Run", {
    refresh(frm) {
        if (frm.is_new()) {
            return;
        }

        frm.add_custom_button(__("Generate Payment Entry Draft Lines"), function () {
            frappe.call({
                method: "ndis_crm.phase28_payment_entry_draft.generate_payment_entry_draft_lines",
                args: { payment_entry_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Generating Payment Entry draft lines...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Payment Entry draft lines generated"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));

        frm.add_custom_button(__("Validate Payment Entry Draft Readiness"), function () {
            frappe.call({
                method: "ndis_crm.phase28_payment_entry_draft.validate_payment_entry_draft_readiness",
                args: { payment_entry_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Validating Payment Entry draft readiness...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Payment Entry draft readiness validated"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));

        frm.add_custom_button(__("Mark Ready for Payment Entry Draft Creation"), function () {
            frappe.call({
                method: "ndis_crm.phase28_payment_entry_draft.mark_ready_for_payment_entry_draft_creation",
                args: { payment_entry_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Marking ready for Payment Entry draft creation...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Ready for Payment Entry draft creation"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));

        frm.add_custom_button(__("Approve Payment Entry Draft Run"), function () {
            frappe.call({
                method: "ndis_crm.phase28_payment_entry_draft.approve_payment_entry_draft_run",
                args: { payment_entry_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Approving Payment Entry Draft Run...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Payment Entry Draft Run approved"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));

        frm.add_custom_button(__("Create Draft Payment Entries"), function () {
            frappe.confirm(
                __("This will create Draft Payment Entry records only. It will not submit Payment Entry, create GL, journal, write-off, recovery, or bank reconciliation."),
                function () {
                    frappe.call({
                        method: "ndis_crm.phase28_payment_entry_draft.create_draft_payment_entries",
                        args: { payment_entry_draft_run: frm.doc.name },
                        freeze: true,
                        freeze_message: __("Creating Draft Payment Entries...")
                    }).then((r) => {
                        if (r.message) {
                            frappe.show_alert({ message: r.message.message || __("Draft Payment Entries created"), indicator: "green" });
                            frm.reload_doc();
                        }
                    });
                }
