...
def install():
    ...
    create_recovery_outcome_action_draft_doctypes()
    create_custom_fields_phase48()
    create_optional_action_target_custom_fields()
    create_form_scripts()
    frappe.clear_cache()
    frappe.db.commit()
    print("NDIS CRM Phase 48 Recovery Outcome Action Draft gate installed successfully.")
...
def repair_partial_crm_deal_phase48_fields():
    expected = {
    ...
    for fieldname, values in expected.items():
        ...
        if changed:
            ...
            print(f"Repaired CRM Deal Phase 48 field: {custom_field_name}")
...
def ensure_required_doctypes():
    ...
    if missing:
        frappe.throw("Missing required Phase 48 DocTypes: " + ", ".join(missing))

    print("Required Phase 48 DocTypes found.")
...
def create_recovery_outcome_action_draft_doctypes():
    create_recovery_outcome_action_draft_line()
    create_recovery_outcome_action_draft_run()


def create_recovery_outcome_action_draft_line():
    fields = [
        make_field("Source", "source_section", "Section Break"),
        make_field("Recovery Outcome Action Draft Source Key", "recovery_outcome_action_draft_source_key", "Data", read_only=1),

        make_field("Draft Targets", "draft_targets_section", "Section Break"),
        make_field("Target Action Mode Snapshot", "target_action_mode_snapshot", "Data", read_only=1),
        make_field("Target Action DocType", "target_action_doctype", "Data", read_only=1),
        make_field("Target Action Name", "target_action_name", "Data", read_only=1),
        make_field("Target Action Status", "target_action_status", "Data", read_only=1),
        ...
        make_field("Earlier Target Snapshot", "earlier_target_snapshot_section", "Section Break"),
        ...
        make_field("Source Target ToDo", "source_target_todo", "Data", read_only=1),
        make_field("Source Target ToDo Status", "source_target_todo_status", "Data", read_only=1),
        make_field("Source Target Task", "source_target_task", "Data", read_only=1),
        make_field("Source Target Task Status", "source_target_task_status", "Data", read_only=1),
        ...
        make_field("Action Draft", "action_draft_section", "Section Break"),
        make_field("Recovery Outcome Action Draft Source Ready", "recovery_outcome_action_draft_source_ready", "Check", default="0"),
        ...
        make_field("Action Draft Review Complete", "action_draft_review_complete", "Check", default="0"),
        make_field("Action Draft Authorized", "action_draft_authorized", "Check", default="0"),
        make_field("ToDo Draft Authorized", "todo_draft_authorized", "Check", default="0"),
        make_field("Task Draft Authorized", "task_draft_authorized", "Check", default="0"),

        make_field("Blocked Activation / Downstream Controls", "blocked_downstream_section", "Section Break"),
        make_field("ToDo Activation Authorized", "todo_activation_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Task Activation Authorized", "task_activation_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Task Completion Authorized", "task_completion_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Communication Creation Authorized", "communication_creation_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Email Send Authorized", "email_send_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Event Creation Authorized", "event_creation_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Recovery Case Creation Authorized", "recovery_case_creation_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Journal Entry Authorized", "journal_entry_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Manual GL Authorized", "manual_gl_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Payment Entry Authorized", "payment_entry_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Sales Invoice Authorized", "sales_invoice_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Adjustment Authorized", "adjustment_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Bank Reconciliation Authorized", "bank_reconciliation_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Claim Batch Authorized", "claim_batch_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Claim Line Authorized", "claim_line_authorized", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Remittance Import Authorized", "remittance_import_authorized", "Check", default="0", description="Blocked in Phase 48."),

        make_field("Line Status", "line_status_section", "Section Break"),
        make_field("Recovery Outcome Action Draft Hold", "recovery_outcome_action_draft_hold", "Check", default="1", in_list_view=1),
        make_field("Recovery Outcome Action Draft Hold Reason", "recovery_outcome_action_draft_hold_reason", "Small Text"),
        make_field("Line Ready for Recovery Outcome Action Draft", "line_ready_for_recovery_outcome_action_draft", "Check", default="0", in_list_view=1),
        make_field("Recovery Outcome Action Draft Line Status", "recovery_outcome_action_draft_line_status", "Select", options="Draft\nReady\nApproved\nCRM Action Draft Prepared\nDraft ToDo Created\nDraft Task Created\nDraft ToDo and Task Created\nReturned\nCancelled", default="Draft", in_list_view=1),
        make_field("Notes", "notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=RECOVERY_OUTCOME_ACTION_DRAFT_LINE,
        fields=fields,
        istable=1,
    )


def create_recovery_outcome_action_draft_run():
    fields = [
        make_field("Series", "naming_series", "Select", options="NDIS-RECOVERY-ACTION-DRAFT-.YYYY.-.#####", default="NDIS-RECOVERY-ACTION-DRAFT-.YYYY.-.#####", reqd=1),

        make_field("Status", "status_section", "Section Break"),
        make_field("Status", "status", "Select", options="Draft\nIn Review\nReady for Action Draft Creation\nAction Draft Creation Approved\nAction Drafts Created\nReturned to Action Preparation\nCancelled", default="Draft", in_list_view=1),
        make_field("Readiness %", "readiness_percent", "Percent", read_only=1, in_list_view=1),
        make_field("Recovery Outcome Action Draft Run Ready", "recovery_outcome_action_draft_run_ready", "Check", read_only=1, in_list_view=1),
        make_field("No Recovery Outcome Action Draft Candidate Found", "no_recovery_outcome_action_draft_candidate_found", "Check", default="0", read_only=1),

        make_field("Target Action Draft Mode", "target_action_draft_mode", "Select", options="CRM Action Draft Only\nDraft ToDo If Available\nDraft Task If Available\nDraft ToDo and Task If Available", default="CRM Action Draft Only"),
        ...
        make_field("Action Draft Creation Allowed", "action_draft_creation_allowed", "Check", default="0"),
        ...
        make_field("Action Activation Allowed", "action_activation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Action Completion Allowed", "action_completion_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Communication Creation Allowed", "communication_creation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Email Send Allowed", "email_send_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Event Creation Allowed", "event_creation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Recovery Case Creation Allowed", "recovery_case_creation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Journal Entry Creation Allowed", "journal_entry_creation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Manual GL Creation Allowed", "manual_gl_creation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Payment Entry Creation Allowed", "payment_entry_creation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Sales Invoice Creation Allowed", "sales_invoice_creation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Adjustment Creation Allowed", "adjustment_creation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Bank Reconciliation Allowed", "bank_reconciliation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Claim Batch Creation Allowed", "claim_batch_creation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Claim Line Creation Allowed", "claim_line_creation_allowed", "Check", default="0", description="Blocked in Phase 48."),
        make_field("Remittance Import Creation Allowed", "remittance_import_creation_allowed", "Check", default="0", description="Blocked in Phase 48."),

        make_field("Summary", "summary_section", "Section Break"),
        make_field("Recovery Outcome Action Draft Line Count", "recovery_outcome_action_draft_line_count", "Int", read_only=1),
        make_field("Recovery Outcome Action Draft Amount Total", "recovery_outcome_action_draft_amount_total", "Currency", read_only=1),
        make_field("Recovery Outcome Action Draft Ready Count", "recovery_outcome_action_draft_ready_count", "Int", read_only=1),
        make_field("Recovery Outcome Action Draft Hold Count", "recovery_outcome_action_draft_hold_count", "Int", read_only=1),
        make_field("CRM Action Draft Count", "crm_action_draft_count", "Int", read_only=1),
        make_field("ToDo Draft Count", "todo_draft_count", "Int", read_only=1),
        make_field("Task Draft Count", "task_draft_count", "Int", read_only=1),
        make_field("Action Draft Created Count", "action_draft_created_count", "Int", read_only=1),
        ...
        make_field("Blocked Task Completion Count", "blocked_task_completion_count", "Int", read_only=1),
    ...
    fields += [
        ...
        make_field("NDIS CRM Recovery Outcome Action Preparation Run", "recovery_outcome_action_preparation_run", "Link", options=RECOVERY_OUTCOME_ACTION_PREPARATION_RUN, reqd=1, in_list_view=1),
        ...
        make_field("Recovery Outcome Action Draft Owner", "recovery_outcome_action_draft_owner", "Link", options="User", in_list_view=1),
        make_field("Recovery Outcome Action Preparation Owner", "recovery_outcome_action_preparation_owner", "Link", options="User"),
        make_field("Created By", "created_by", "Link", options="User", read_only=1),
        make_field("Created On", "created_on", "Datetime", read_only=1),

        make_field("Recovery Outcome Action Draft Lines", "recovery_outcome_action_draft_lines_section", "Section Break"),
        make_field("Recovery Outcome Action Draft Lines", "recovery_outcome_action_draft_lines", "Table", options=RECOVERY_OUTCOME_ACTION_DRAFT_LINE),

        make_field("Notes", "notes_section", "Section Break"),
        make_field("Recovery Outcome Action Draft Notes", "recovery_outcome_action_draft_notes", "Small Text"),
        make_field("Returned / Blocker Notes", "returned_notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=RECOVERY_OUTCOME_ACTION_DRAFT_RUN,
        fields=fields,
...
def create_custom_fields_phase48():
    deal_fields = [
        {
            "fieldname": "recovery_outcome_action_draft_section",
            "label": "NDIS Recovery Outcome Action Draft Run",
            "fieldtype": "Section Break",
            "insert_after": "recovery_outcome_action_preparation_ready",
        },
        {
            "fieldname": "ndis_recovery_outcome_action_draft_run_required",
            "label": "Recovery Outcome Action Draft Required Before Active Deal",
            "fieldtype": "Check",
            "default": "0",
            "insert_after": "recovery_outcome_action_draft_section",
        },
        {
            "fieldname": "ndis_recovery_outcome_action_draft_run",
            "label": "NDIS CRM Recovery Outcome Action Draft Run",
            "fieldtype": "Small Text",
            "read_only": 1,
            "insert_after": "ndis_recovery_outcome_action_draft_run_required",
        },
        {
            "fieldname": "recovery_outcome_action_draft_status",
            "label": "Recovery Outcome Action Draft Status",
            "fieldtype": "Small Text",
            "read_only": 1,
            "insert_after": "ndis_recovery_outcome_action_draft_run",
        },
        {
            "fieldname": "recovery_outcome_action_draft_ready",
            "label": "Recovery Outcome Action Draft Ready",
            "fieldtype": "Check",
            "read_only": 1,
            "insert_after": "recovery_outcome_action_draft_status",
        },
    ]
    ...
    shared_fields = [
        ...
        },
        {
            ...
            "label": "NDIS CRM Recovery Outcome Action Draft Run",
            "fieldtype": "Link",
            "options": RECOVERY_OUTCOME_ACTION_DRAFT_RUN,
            "read_only": 1,
            "insert_after": "recovery_outcome_action_draft_section",
        },
        {
    ...
    repair_partial_crm_deal_phase48_fields()
    safe_create_custom_fields(custom_fields)
    print("Created / updated Phase 48 recovery outcome action draft chain custom fields.")


def create_optional_action_target_custom_fields():
    fields = [
        {
            "fieldname": "ndis_crm_recovery_outcome_action_draft_section",
            "label": "NDIS CRM Recovery Outcome Action Draft Control",
            "fieldtype": "Section Break",
            "insert_after": "modified",
        },
        {
            "fieldname": "ndis_crm_recovery_outcome_action_draft_run",
            "label": "NDIS CRM Recovery Outcome Action Draft Run",
            "fieldtype": "Link",
            "options": RECOVERY_OUTCOME_ACTION_DRAFT_RUN,
            "read_only": 1,
            "insert_after": "ndis_crm_recovery_outcome_action_draft_section",
        },
        {
            "fieldname": "ndis_crm_recovery_outcome_action_draft_line",
            "label": "NDIS CRM Recovery Outcome Action Draft Line",
            "fieldtype": "Data",
            "read_only": 1,
            "insert_after": "ndis_crm_recovery_outcome_action_draft_run",
        },
        {
            "fieldname": "ndis_crm_action_draft_status",
            "label": "NDIS CRM Action Draft Status",
            "fieldtype": "Data",
            "read_only": 1,
            "insert_after": "ndis_crm_recovery_outcome_action_draft_line",
        },
        {
            "fieldname": "ndis_crm_action_draft_blocked",
            "label": "NDIS CRM Action Draft Blocked",
            "fieldtype": "Check",
            "default": "1",
            "insert_after": "ndis_crm_action_draft_status",
        },
        {
            "fieldname": "ndis_crm_action_activation_blocked",
            "label": "NDIS CRM Action Activation Blocked",
            "fieldtype": "Check",
            "default": "1",
            "insert_after": "ndis_crm_action_draft_blocked",
        },
        {
            "fieldname": "ndis_crm_action_completion_blocked",
            "label": "NDIS CRM Action Completion Blocked",
            "fieldtype": "Check",
            "default": "1",
            "insert_after": "ndis_crm_action_activation_blocked",
        },
    ]

    if doctype_exists(TODO):
        safe_create_custom_fields({TODO: fields})
        print("Created / updated optional ToDo Phase 48 draft marker fields.")
    else:
    ...
    if doctype_exists(TASK):
        ...
        print("Created / updated optional Task Phase 48 draft marker fields.")
    else:
...
def _phase48_deal_actions():
    return r'''
// NDIS CRM Phase 48 Deal Actions
{
  label: "Create Recovery Outcome Action Draft Run",
  onClick: () => {
    call("ndis_crm.phase48_recovery_outcome_action_draft.create_recovery_outcome_action_draft_run_from_crm_deal", {
      deal: doc.name
    }).then((data) => {
      if (data && data.name) {
        createToast({
          title: data.created ? "Recovery Outcome Action Draft Run Created" : "Existing Recovery Outcome Action Draft Run Found",
          icon: "check",
          iconClasses: "text-green-600",
        })
        window.open(`/app/ndis-crm-recovery-outcome-action-draft-run/${data.name}`, "_blank")
      }
    })
  }
},
{
  label: "Open Recovery Outcome Action Draft Run",
  onClick: () => {
    if (doc.ndis_recovery_outcome_action_draft_run) {
      window.open(`/app/ndis-crm-recovery-outcome-action-draft-run/${doc.ndis_recovery_outcome_action_draft_run}`, "_blank")
    } else {
      createToast({ title: "No Recovery Outcome Action Draft Run linked yet", icon: "info" })
    }
  }
}
'''.strip()
...
def _deal_script():
    try:
        from ndis_crm.setup.phase47_recovery_outcome_action_preparation import _deal_script as phase47_deal_script

        script = phase47_deal_script()
    except Exception:
        script = '''
...
'''.strip()
    ...
    if "NDIS CRM Phase 48 Deal Actions" in script:
        return script

    return _append_before_last(script, "\n    ]", _phase48_deal_actions())
...
def create_form_scripts():
    ...
    prep_script = r'''
frappe.ui.form.on("NDIS CRM Recovery Outcome Action Preparation Run", {
    refresh(frm) {
        if (frm.is_new()) {
            return;
        }

        frm.add_custom_button(__("Create Recovery Outcome Action Draft Run"), function () {
            frappe.call({
                method: "ndis_crm.phase48_recovery_outcome_action_draft.create_recovery_outcome_action_draft_run_from_action_preparation_run",
                args: { recovery_outcome_action_preparation_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Creating recovery outcome action draft run...")
            }).then((r) => {
                if (r.message && r.message.name) {
                    frappe.show_alert({ message: r.message.message || __("Recovery Outcome Action Draft Run created"), indicator: "green" });
                    frm.reload_doc();
                    frappe.set_route("Form", "NDIS CRM Recovery Outcome Action Draft Run", r.message.name);
                }
            });
        }, __("Actions"));

        if (frm.doc.ndis_recovery_outcome_action_draft_run) {
            frm.add_custom_button(__("Open Recovery Outcome Action Draft Run"), function () {
                frappe.set_route("Form", "NDIS CRM Recovery Outcome Action Draft Run", frm.doc.ndis_recovery_outcome_action_draft_run);
            }, __("Open"));
        }
    }
});
...
frappe.ui.form.on("NDIS CRM Recovery Outcome Action Draft Run", {
    refresh(frm) {
        ...
        frm.add_custom_button(__("Generate Draft Lines"), function () {
            frappe.call({
                method: "ndis_crm.phase48_recovery_outcome_action_draft.generate_recovery_outcome_action_draft_lines",
                args: { recovery_outcome_action_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Generating action draft lines...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Action draft lines generated"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));
        ...
        frm.add_custom_button(__("Validate Draft Readiness"), function () {
            frappe.call({
                method: "ndis_crm.phase48_recovery_outcome_action_draft.validate_recovery_outcome_action_draft_readiness",
                args: { recovery_outcome_action_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Validating action draft readiness...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Action draft readiness validated"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));
        ...
        frm.add_custom_button(__("Mark Ready for Draft Creation"), function () {
            frappe.call({
                method: "ndis_crm.phase48_recovery_outcome_action_draft.mark_ready_for_recovery_outcome_action_draft",
                args: { recovery_outcome_action_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Marking ready for action draft creation...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Ready for action draft creation"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));
        ...
        frm.add_custom_button(__("Approve Draft Run"), function () {
            frappe.call({
                method: "ndis_crm.phase48_recovery_outcome_action_draft.approve_recovery_outcome_action_draft_run",
                args: { recovery_outcome_action_draft_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Approving action draft run...")
            }).then((r) => {
                if (r.message) {
                    frappe.show_alert({ message: r.message.message || __("Action Draft Run approved"), indicator: "green" });
                    frm.reload_doc();
                }
            });
        }, __("Actions"));

        frm.add_custom_button(__("Create Action Drafts"), function () {
            frappe.confirm(
                __("This creates CRM action drafts by default, or optional draft-marked ToDo/Task records only when explicitly allowed. It will not activate or complete actions, and will not create Communication, Email Queue, Event, Recovery Case, Journal Entry, GL Entry, Payment Entry, Sales Invoice, adjustment, bank reconciliation, Claim Batch, Claim Line, or Remittance Import."),
                function () {
                    frappe.call({
                        method: "ndis_crm.phase48_recovery_outcome_action_draft.create_recovery_outcome_action_drafts",
                        args: { recovery_outcome_action_draft_run: frm.doc.name },
                        freeze: true,
                        freeze_message: __("Creating action drafts...")
                    }).then((r) => {
                        if (r.message) {
                            frappe.show_alert({ message: r.message.message || __("Action drafts created"), indicator: "green" });
                            frm.reload_doc();
                        }
                    });
                }
        ...
        if (frm.doc.recovery_outcome_action_preparation_run) {
            frm.add_custom_button(__("Open Action Preparation Run"), function () {
                frappe.set_route("Form", "NDIS CRM Recovery Outcome Action Preparation Run", frm.doc.recovery_outcome_action_preparation_run);
            }, __("Open"));
        }
...
'''.strip()
    ...
    if frappe.db.exists("DocType", "Client Script"):
        upsert_doc(
            ...
            {
                "dt": RECOVERY_OUTCOME_ACTION_PREPARATION_RUN,
                "view": "Form",
                "enabled": 1,
                "script": prep_script,
            },
