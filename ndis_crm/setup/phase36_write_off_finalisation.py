...
def install():
    ...
    create_write_off_finalisation_doctypes()
    create_custom_fields_phase36()
    create_journal_entry_custom_fields()
    create_form_scripts()
    frappe.clear_cache()
    frappe.db.commit()
    print("NDIS CRM Phase 36 Write Off Finalisation bridge installed successfully.")
...
def ensure_required_doctypes():
    ...
    if missing:
        frappe.throw("Missing required Phase 36 DocTypes: " + ", ".join(missing))

    print("Required Phase 36 DocTypes found.")
...
def create_write_off_finalisation_doctypes():
    create_write_off_finalisation_line()
    create_write_off_finalisation_run()


def create_write_off_finalisation_line():
    fields = [
        make_field("Source", "source_section", "Section Break"),
        make_field("Write Off Finalisation Source Key", "write_off_finalisation_source_key", "Data", read_only=1),

        make_field("Journal Entry / GL Outcome", "journal_entry_section", "Section Break"),
        ...
        make_field("Write Off Values", "write_off_values_section", "Section Break"),
        ...
        make_field("Finalised Write Off Amount", "finalised_write_off_amount", "Currency", in_list_view=1),
        ...
        make_field("Account Snapshot", "account_snapshot_section", "Section Break"),
        ...
        make_field("Write Off Finalisation Source Ready", "write_off_finalisation_source_ready", "Check", default="0"),
        make_field("Journal Entry Posting Verified", "journal_entry_posting_verified", "Check", default="0"),
        make_field("Standard GL Review Complete", "standard_gl_review_complete", "Check", default="0"),
        make_field("Write Off Finalisation Authorized", "write_off_finalisation_authorized", "Check", default="0"),
        make_field("Manual GL Authorized", "manual_gl_authorized", "Check", default="0", description="Blocked in Phase 36."),
        make_field("Payment Entry Authorized", "payment_entry_authorized", "Check", default="0", description="Blocked in Phase 36."),
        make_field("Additional Journal Entry Authorized", "additional_journal_entry_authorized", "Check", default="0", description="Blocked in Phase 36."),
        make_field("Recovery Authorized", "recovery_authorized", "Check", default="0", description="Blocked in Phase 36."),
        make_field("Adjustment Authorized", "adjustment_authorized", "Check", default="0", description="Blocked in Phase 36."),
        make_field("Bank Reconciliation Authorized", "bank_reconciliation_authorized", "Check", default="0", description="Blocked in Phase 36."),
        make_field("Write Off Finalisation Hold", "write_off_finalisation_hold", "Check", default="1", in_list_view=1),
        make_field("Write Off Finalisation Hold Reason", "write_off_finalisation_hold_reason", "Small Text"),

        make_field("Line Status", "line_status_section", "Section Break"),
        make_field("Line Ready for Write Off Finalisation", "line_ready_for_write_off_finalisation", "Check", default="0", in_list_view=1),
        make_field("Write Off Finalisation Line Status", "write_off_finalisation_line_status", "Select", options="Draft\nReady\nApproved\nWrite Off Finalised\nReturned\nCancelled", default="Draft", in_list_view=1),
        make_field("Notes", "notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=WRITE_OFF_FINALISATION_LINE,
        fields=fields,
        istable=1,
    )


def create_write_off_finalisation_run():
    fields = [
        make_field("Series", "naming_series", "Select", options="NDIS-WRITEOFF-FIN-.YYYY.-.#####", default="NDIS-WRITEOFF-FIN-.YYYY.-.#####", reqd=1),

        make_field("Status", "status_section", "Section Break"),
        make_field("Status", "status", "Select", options="Draft\nIn Review\nReady for Write Off Finalisation\nWrite Off Finalisation Approved\nWrite Off Finalised\nReturned to JE Submission\nCancelled", default="Draft", in_list_view=1),
        make_field("Readiness %", "readiness_percent", "Percent", read_only=1, in_list_view=1),
        make_field("Write Off Finalisation Run Ready", "write_off_finalisation_run_ready", "Check", read_only=1, in_list_view=1),
        make_field("No Write Off Finalisation Candidate Found", "no_write_off_finalisation_candidate_found", "Check", default="0", read_only=1),
        make_field("Write Off Finalisation Allowed", "write_off_finalisation_allowed", "Check", default="0"),
        make_field("Manual GL Creation Allowed", "manual_gl_creation_allowed", "Check", default="0", description="Blocked in Phase 36."),
        make_field("Payment Entry Creation Allowed", "payment_entry_creation_allowed", "Check", default="0", description="Blocked in Phase 36."),
        make_field("Additional Journal Entry Creation Allowed", "additional_journal_entry_creation_allowed", "Check", default="0", description="Blocked in Phase 36."),
        make_field("Recovery Creation Allowed", "recovery_creation_allowed", "Check", default="0", description="Blocked in Phase 36."),
        make_field("Adjustment Creation Allowed", "adjustment_creation_allowed", "Check", default="0", description="Blocked in Phase 36."),
        make_field("Bank Reconciliation Allowed", "bank_reconciliation_allowed", "Check", default="0", description="Blocked in Phase 36."),

        make_field("Summary", "summary_section", "Section Break"),
        make_field("Write Off Finalisation Line Count", "write_off_finalisation_line_count", "Int", read_only=1),
        make_field("Finalised Write Off Amount Total", "finalised_write_off_amount_total", "Currency", read_only=1),
        make_field("Write Off Finalisation Ready Count", "write_off_finalisation_ready_count", "Int", read_only=1),
        make_field("Write Off Finalisation Hold Count", "write_off_finalisation_hold_count", "Int", read_only=1),
        make_field("Submitted Journal Entry Count", "submitted_journal_entry_count", "Int", read_only=1),
        make_field("Standard GL Entry Count", "standard_gl_entry_count", "Int", read_only=1),
        make_field("Write Off Finalised Line Count", "write_off_finalised_line_count", "Int", read_only=1),
        ...
        make_field("Blocked Additional Journal Entry Count", "blocked_additional_journal_entry_count", "Int", read_only=1),
    ...
    fields += [
        ...
        make_field("NDIS CRM Write Off JE Submission Run", "write_off_je_submission_run", "Link", options=WRITE_OFF_JE_SUBMISSION_RUN, reqd=1, in_list_view=1),
    ...
    fields += [
        ...
        make_field("Write Off Finalisation Owner", "write_off_finalisation_owner", "Link", options="User", in_list_view=1),
        make_field("Write Off JE Submission Owner", "write_off_je_submission_owner", "Link", options="User"),
        ...
        make_field("Write Off Finalisation Lines", "write_off_finalisation_lines_section", "Section Break"),
        make_field("Write Off Finalisation Lines", "write_off_finalisation_lines", "Table", options=WRITE_OFF_FINALISATION_LINE),

        make_field("Notes", "notes_section", "Section Break"),
        make_field("Write Off Finalisation Notes", "write_off_finalisation_notes", "Small Text"),
        make_field("Returned / Blocker Notes", "returned_notes", "Small Text"),
    ]

    create_doctype_if_missing(
        name=WRITE_OFF_FINALISATION_RUN,
        fields=fields,
...
def create_custom_fields_phase36():
    deal_fields = [
        {
            "fieldname": "write_off_finalisation_section",
            "label": "NDIS Write Off Finalisation Run",
            "fieldtype": "Section Break",
            "insert_after": "write_off_je_submission_ready",
        },
        {
            "fieldname": "ndis_write_off_finalisation_run_required",
            "label": "Write Off Finalisation Required Before Active Deal",
            "fieldtype": "Check",
            "default": "0",
            "insert_after": "write_off_finalisation_section",
            "description": "Optional guard only. Write-off finalisation normally happens after JE submission.",
        },
        {
            "fieldname": "ndis_write_off_finalisation_run",
            "label": "NDIS CRM Write Off Finalisation Run",
            "fieldtype": "Link",
            "options": WRITE_OFF_FINALISATION_RUN,
            "read_only": 1,
            "insert_after": "ndis_write_off_finalisation_run_required",
        },
        {
            "fieldname": "write_off_finalisation_status",
            "label": "Write Off Finalisation Status",
            "fieldtype": "Data",
            "read_only": 1,
            "insert_after": "ndis_write_off_finalisation_run",
        },
        {
            "fieldname": "write_off_finalisation_ready",
            "label": "Write Off Finalisation Ready",
            "fieldtype": "Check",
            "read_only": 1,
            "insert_after": "write_off_finalisation_status",
        },
    ]
    ...
    print("Created / updated Phase 36 write-off finalisation chain custom fields.")
...
def create_journal_entry_custom_fields():
    custom_fields = {
        "Journal Entry": [
            {
                "fieldname": "ndis_crm_write_off_finalisation_section",
                "label": "NDIS CRM Write Off Finalisation Control",
                "fieldtype": "Section Break",
                "insert_after": "ndis_crm_submission_gate_status",
            },
            {
                "fieldname": "ndis_crm_write_off_finalisation_run",
                "label": "NDIS CRM Write Off Finalisation Run",
                "fieldtype": "Link",
                "options": WRITE_OFF_FINALISATION_RUN,
                "read_only": 1,
                "insert_after": "ndis_crm_write_off_finalisation_section",
            },
            {
                "fieldname": "ndis_crm_write_off_finalisation_line",
                "label": "NDIS CRM Write Off Finalisation Line",
                "fieldtype": "Data",
                "read_only": 1,
                "insert_after": "ndis_crm_write_off_finalisation_run",
            },
            {
                "fieldname": "ndis_crm_write_off_finalisation_status",
                "label": "NDIS CRM Write Off Finalisation Status",
                "fieldtype": "Data",
                "read_only": 1,
                "insert_after": "ndis_crm_write_off_finalisation_line",
            },
        ]
    }

    create_custom_fields(custom_fields, update=True)
    print("Created / updated Phase 36 Journal Entry finalisation marker custom fields.")
...
def _phase36_deal_actions():
    return r'''
// NDIS CRM Phase 36 Deal Actions
{
  label: "Create Write Off Finalisation Run",
  onClick: () => {
    call("ndis_crm.phase36_write_off_finalisation.create_write_off_finalisation_run_from_crm_deal", {
      deal: doc.name
    }).then((data) => {
      if (data && data.name) {
        createToast({
          title: data.created ? "Write Off Finalisation Run Created" : "Existing Write Off Finalisation Run Found",
          icon: "check",
          iconClasses: "text-green-600",
        })
        window.open(`/app/ndis-crm-write-off-finalisation-run/${data.name}`, "_blank")
      }
    })
  }
},
{
  label: "Open Write Off Finalisation Run",
  onClick: () => {
    if (doc.ndis_write_off_finalisation_run) {
      window.open(`/app/ndis-crm-write-off-finalisation-run/${doc.ndis_write_off_finalisation_run}`, "_blank")
    } else {
      createToast({ title: "No Write Off Finalisation Run linked yet", icon: "info" })
    }
  }
}
'''.strip()
...
def _deal_script():
    try:
        from ndis_crm.setup.phase35_write_off_je_submission import _deal_script as phase35_deal_script
        script = phase35_deal_script()
    except Exception:
        script = '''
...
'''.strip()
    ...
    if "NDIS CRM Phase 36 Deal Actions" in script:
        return script

    return _append_before_last(script, "\n    ]", _phase36_deal_actions())
...
frappe.ui.form.on("NDIS CRM Write Off JE Submission Run", {
    refresh(frm) {
        ...
        frm.add_custom_button(__("Create Write Off Finalisation Run"), function () {
            frappe.call({
                method: "ndis_crm.phase36_write_off_finalisation.create_write_off_finalisation_run_from_je_submission_run",
                args: { write_off_je_submission_run: frm.doc.name },
                freeze: true,
                freeze_message: __("Creating write-off finalisation run...")
            }).then((r) => {
                if (r.message && r.message.name) {
                    frappe.show_alert({ message: r.message.message || __("Write Off Finalisation Run created"), indicator: "green" });
                    frm.reload_doc();
                    frappe.set_route("Form", "NDIS CRM Write Off Finalisation Run", r.message.name);
                }
            });
        }, __("Actions"));

        if (frm.doc.ndis_write_off_finalisation_run) {
            frm.add_custom_button(__("Open Write Off Finalisation Run"), function () {
                frappe.set_route("Form", "NDIS CRM Write Off Finalisation Run", frm.doc.ndis_write_off_finalisation_run);
            }, __("Open"));
        }
    }
});
