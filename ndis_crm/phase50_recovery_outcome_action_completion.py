...
def _check_role():
    if not set(frappe.get_roles()).intersection(ALLOWED_ROLES):
        frappe.throw(_("You do not have permission to perform this recovery outcome action completion action."))
...
def _target_mode_needs_todo(mode):
    return mode in ["Complete Released ToDo If Available", "Complete Released ToDo and Task If Available"]


def _target_mode_needs_task(mode):
    return mode in ["Complete Released Task If Available", "Complete Released ToDo and Task If Available"]


def _existing_run_for_action_activation_run(recovery_outcome_action_activation_run):
    if not _doctype_exists(RECOVERY_OUTCOME_ACTION_COMPLETION_RUN):
        return None

    if _field_exists(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, "ndis_recovery_outcome_action_completion_run"):
    ...
    return frappe.db.get_value(
        ...
        {"recovery_outcome_action_activation_run": recovery_outcome_action_activation_run},
        "name",
    )
...
def _existing_run_for_deal(deal):
    ...
    if _field_exists(CRM_DEAL, "ndis_recovery_outcome_action_completion_run"):
        existing = frappe.db.get_value(CRM_DEAL, deal, "ndis_recovery_outcome_action_completion_run")
        if existing:
            return existing
...
def _get_action_activation_run_for_deal(deal):
    if _field_exists(CRM_DEAL, "ndis_recovery_outcome_action_activation_run"):
        run = frappe.db.get_value(CRM_DEAL, deal, "ndis_recovery_outcome_action_activation_run")
        if run:
            return run

    return frappe.db.get_value(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, {"crm_deal": deal}, "name")


def _is_action_activation_completed(run):
    if not run or not frappe.db.exists(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, run):
        return False

    status, ready = frappe.db.get_value(
        RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN,
        run,
        ["status", "recovery_outcome_action_activation_run_ready"],
    )

    return status == "Actions Activated" and bool(ready)


def _is_action_completion_approved(run):
...
def _append_line_if_missing(doc, row_data):
    existing = {
        row.recovery_outcome_action_completion_source_key
        for row in doc.get("recovery_outcome_action_completion_lines") or []
        if row.get("recovery_outcome_action_completion_source_key")
    }

    key = row_data.get("recovery_outcome_action_completion_source_key")

    if key and key in existing:
        return False

    doc.append("recovery_outcome_action_completion_lines", row_data)
    return True


def _source_key(row):
    return row.get("recovery_outcome_action_activation_source_key") or "|".join([
...
def _is_action_activation_line_done(row):
    return row.get("recovery_outcome_action_activation_line_status") in [
...
def _target_doc_is_phase49_released(doctype, name, draft_run, activation_run):
    if not doctype or not name or not draft_run or not activation_run:
        return False

    if not _doctype_exists(doctype) or not frappe.db.exists(doctype, name):
        return False

    if not _field_exists(doctype, "ndis_crm_recovery_outcome_action_draft_run"):
        return False

    if not _field_exists(doctype, "ndis_crm_recovery_outcome_action_activation_run"):
        return False

    linked_draft = frappe.db.get_value(doctype, name, "ndis_crm_recovery_outcome_action_draft_run")
    linked_activation = frappe.db.get_value(doctype, name, "ndis_crm_recovery_outcome_action_activation_run")

    return linked_draft == draft_run and linked_activation == activation_run


def _build_completion_line(row, source_doc):
    source_ready = bool(
        source_doc.get("status") == "Actions Activated"
        and _is_action_activation_line_done(row)
        and row.get("recovery_outcome_action_activation_source_ready")
        and row.get("action_activation_review_complete")
        and row.get("action_activation_authorized")
        and row.get("line_ready_for_recovery_outcome_action_activation")
        and not row.get("action_completion_authorized")
        ...
        and not row.get("recovery_outcome_action_activation_hold")
        and _to_float(row.get("proposed_recovery_amount")) > 0
    )

    return {
        "recovery_outcome_action_completion_source_key": _source_key(row),
        ...
        "target_completion_mode_snapshot": None,
        ...
        "recovery_outcome_action_completion_source_ready": 1 if source_ready else 0,
        "action_type": row.get("action_type"),
        "action_route": row.get("action_route"),
        "action_priority": row.get("action_priority") or "Normal",
        "action_owner": row.get("action_owner") or source_doc.get("recovery_outcome_action_activation_owner") or frappe.session.user,
        ...
        "action_completion_review_complete": 0,
        "action_completion_authorized": 0,
        "todo_completion_authorized": 0,
        ...
        "recovery_outcome_action_completion_hold": 0 if source_ready else 1,
        "recovery_outcome_action_completion_hold_reason": None if source_ready else "Phase 49 action activation source is not ready for completion.",
        "line_ready_for_recovery_outcome_action_completion": 0,
        "recovery_outcome_action_completion_line_status": "Draft",
        "notes": row.get("notes"),
    }


def _generate_lines_from_action_activation_run(doc, source):
    created = 0

    for row in source.get("recovery_outcome_action_activation_lines") or []:
        if not _is_action_activation_line_done(row):
            continue

        if _to_float(row.get("proposed_recovery_amount")) <= 0:
            continue

        if _append_line_if_missing(doc, _build_completion_line(row, source)):
            created += 1
...
def _active_lines(doc):
    return [
        row for row in doc.get("recovery_outcome_action_completion_lines") or []
        if _to_float(row.get("proposed_recovery_amount")) > 0
    ]
...
def _calculate_totals(doc):
    totals = {
        "recovery_outcome_action_completion_line_count": 0,
        "recovery_outcome_action_completion_amount_total": 0,
        "recovery_outcome_action_completion_ready_count": 0,
        "recovery_outcome_action_completion_hold_count": 0,
        "crm_action_completed_count": 0,
    ...
    for row in doc.get("recovery_outcome_action_completion_lines") or []:
        totals["recovery_outcome_action_completion_line_count"] += 1
        totals["recovery_outcome_action_completion_amount_total"] += _to_float(row.get("proposed_recovery_amount"))

        if row.get("line_ready_for_recovery_outcome_action_completion"):
            totals["recovery_outcome_action_completion_ready_count"] += 1

        if row.get("recovery_outcome_action_completion_hold"):
            totals["recovery_outcome_action_completion_hold_count"] += 1

        status = row.get("recovery_outcome_action_completion_line_status")
        if status == "CRM Action Completed":
            totals["crm_action_completed_count"] += 1
            totals["action_completed_count"] += 1
        elif status == "Released ToDo Completed":
            totals["todo_completed_count"] += 1
            totals["action_completed_count"] += 1
        elif status == "Released Task Completed":
            totals["task_completed_count"] += 1
            totals["action_completed_count"] += 1
        elif status == "Released ToDo and Task Completed":
            totals["todo_completed_count"] += 1
            totals["task_completed_count"] += 1
            totals["action_completed_count"] += 1
    ...
    totals["recovery_outcome_action_completion_amount_total"] = round(
        totals["recovery_outcome_action_completion_amount_total"],
        2,
    )
...
def _sync_totals(doc):
    ...
    for fieldname, value in totals.items():
        if _field_exists(RECOVERY_OUTCOME_ACTION_COMPLETION_RUN, fieldname):
            doc.set(fieldname, value)
...
def _calculate_readiness(doc):
    ...
    checks.append({
        "label": "Recovery Outcome Action Activation Run linked",
        "complete": bool(doc.get("recovery_outcome_action_activation_run")),
    })

    checks.append({
        "label": "Recovery outcome actions activated",
        "complete": _is_action_activation_completed(doc.get("recovery_outcome_action_activation_run")),
    })
    ...
    checks.append({
        "label": "Recovery Outcome Action Completion Owner assigned",
        "complete": bool(doc.get("recovery_outcome_action_completion_owner")),
    })

    mode = doc.get("target_action_completion_mode") or "CRM Action Completion Only"

    if _target_mode_needs_todo(mode):
        checks.append({
            "label": "ToDo DocType exists for ToDo completion mode",
            "complete": _doctype_exists(TODO),
        })

    if _target_mode_needs_task(mode):
        checks.append({
            "label": "Task DocType exists for Task completion mode",
            "complete": _doctype_exists(TASK),
        })

    lines = _active_lines(doc)
    no_candidates = bool(doc.get("no_recovery_outcome_action_completion_candidate_found"))

    checks.append({
        "label": "Action completion lines exist or no-candidate flag is set",
        "complete": bool(lines) or no_candidates,
    })

    if lines:
        source_not_ready = [row.service_line for row in lines if not row.get("recovery_outcome_action_completion_source_ready")]
        checks.append({
            "label": "Action completion source-ready flags are complete",
        ...
        review_missing = [row.service_line for row in lines if not row.get("action_completion_review_complete")]
        checks.append({
            "label": "Action completion review complete",
            "complete": not review_missing,
            "details": review_missing,
        })

        auth_missing = [row.service_line for row in lines if not row.get("action_completion_authorized")]
        checks.append({
            "label": "Action completion authorized",
            "complete": not auth_missing,
            "details": auth_missing,
        })

        if _target_mode_needs_todo(mode):
            missing_todo = [row.service_line for row in lines if not row.get("target_todo")]
            checks.append({
                "label": "Existing Phase 49-released ToDo linked for all ToDo completion lines",
                "complete": not missing_todo,
                "details": missing_todo,
            })

            todo_auth_missing = [row.service_line for row in lines if not row.get("todo_completion_authorized")]
            checks.append({
                "label": "ToDo completion authorized for all ToDo completion lines",
                "complete": not todo_auth_missing,
                "details": todo_auth_missing,
            })

        if _target_mode_needs_task(mode):
            missing_task = [row.service_line for row in lines if not row.get("target_task")]
            checks.append({
                "label": "Existing Phase 49-released Task linked for all Task completion lines",
                "complete": not missing_task,
                "details": missing_task,
            })

            task_auth_missing = [row.service_line for row in lines if not row.get("task_completion_authorized")]
            checks.append({
                "label": "Task completion authorized for all Task completion lines",
                "complete": not task_auth_missing,
                "details": task_auth_missing,
            })

        blocked_fields = [
            ("Communication creation remains blocked in Phase 50", "communication_creation_authorized"),
            ("Email sending remains blocked in Phase 50", "email_send_authorized"),
            ("Event creation remains blocked in Phase 50", "event_creation_authorized"),
            ("Recovery Case creation remains blocked in Phase 50", "recovery_case_creation_authorized"),
            ("Journal Entry authorization remains blocked in Phase 50", "journal_entry_authorized"),
            ("Manual GL authorization remains blocked in Phase 50", "manual_gl_authorized"),
            ("Payment Entry authorization remains blocked in Phase 50", "payment_entry_authorized"),
            ("Sales Invoice authorization remains blocked in Phase 50", "sales_invoice_authorized"),
            ("Adjustment authorization remains blocked in Phase 50", "adjustment_authorized"),
            ("Bank reconciliation authorization remains blocked in Phase 50", "bank_reconciliation_authorized"),
            ("Claim Batch authorization remains blocked in Phase 50", "claim_batch_authorized"),
            ("Claim Line authorization remains blocked in Phase 50", "claim_line_authorized"),
            ("Remittance Import authorization remains blocked in Phase 50", "remittance_import_authorized"),
        ]
        ...
        holds = [row.service_line for row in lines if row.get("recovery_outcome_action_completion_hold")]
        checks.append({
            "label": "No active action completion hold remains",
            "complete": not holds,
            "details": holds,
        })

        not_ready = [row.service_line for row in lines if not row.get("line_ready_for_recovery_outcome_action_completion")]
        checks.append({
            "label": "All active lines marked ready for action completion",
    ...
    return {
        ...
        "recovery_outcome_action_completion_run_ready": total > 0 and complete == total,
...
def _sync_summary_to_links(doc):
    ...
    if _field_exists(RECOVERY_OUTCOME_ACTION_COMPLETION_RUN, "readiness_percent"):
        doc.readiness_percent = summary["readiness_percent"]

    if _field_exists(RECOVERY_OUTCOME_ACTION_COMPLETION_RUN, "recovery_outcome_action_completion_run_ready"):
        doc.recovery_outcome_action_completion_run_ready = 1 if summary["recovery_outcome_action_completion_run_ready"] else 0
    ...
    for doctype, name in targets:
        ...
        _db_set_if_field(doctype, name, "ndis_recovery_outcome_action_completion_run", doc.name)
        _db_set_if_field(doctype, name, "recovery_outcome_action_completion_status", doc.status)
        _db_set_if_field(doctype, name, "recovery_outcome_action_completion_ready", 1 if summary["recovery_outcome_action_completion_run_ready"] else 0)
...
@frappe.whitelist()
def create_recovery_outcome_action_completion_run_from_action_activation_run(recovery_outcome_action_activation_run):
    _check_role()

    if not recovery_outcome_action_activation_run:
        frappe.throw(_("NDIS CRM Recovery Outcome Action Activation Run is required."))

    if not frappe.db.exists(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, recovery_outcome_action_activation_run):
        frappe.throw(_("NDIS CRM Recovery Outcome Action Activation Run {0} was not found.").format(recovery_outcome_action_activation_run))

    existing = _existing_run_for_action_activation_run(recovery_outcome_action_activation_run)
    if existing:
        return {
            "doctype": RECOVERY_OUTCOME_ACTION_COMPLETION_RUN,
            "name": existing,
            "created": False,
            "message": "Existing NDIS CRM Recovery Outcome Action Completion Run returned.",
        }

    source = frappe.get_doc(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, recovery_outcome_action_activation_run)

    doc = frappe.new_doc(RECOVERY_OUTCOME_ACTION_COMPLETION_RUN)
    doc.status = "Draft"
    doc.recovery_outcome_action_activation_run = source.name
    ...
    doc.recovery_outcome_action_completion_owner = frappe.session.user
    doc.recovery_outcome_action_activation_owner = source.get("recovery_outcome_action_activation_owner")
    doc.target_action_completion_mode = "CRM Action Completion Only"
    ...
    doc.todo_completion_allowed = 0
    doc.task_completion_allowed = 0
    ...
    created_count = _generate_lines_from_action_activation_run(doc, source)
    doc.no_recovery_outcome_action_completion_candidate_found = 1 if created_count == 0 else 0
    ...
    return {
        ...
        "name": doc.name,
        "created": True,
        "recovery_outcome_action_completion_line_count": created_count,
        "no_recovery_outcome_action_completion_candidate_found": bool(doc.no_recovery_outcome_action_completion_candidate_found),
        "message": "NDIS CRM Recovery Outcome Action Completion Run created successfully.",
    }


@frappe.whitelist()
def create_recovery_outcome_action_completion_run_from_crm_deal(deal):
    _check_role()
    ...
    source_run = _get_action_activation_run_for_deal(deal)

    if not source_run:
        frappe.throw(_("Please complete NDIS CRM Recovery Outcome Action Activation Run before creating Recovery Outcome Action Completion Run."))

    return create_recovery_outcome_action_completion_run_from_action_activation_run(source_run)


@frappe.whitelist()
def generate_recovery_outcome_action_completion_lines(recovery_outcome_action_completion_run):
    _check_role()

    doc = frappe.get_doc(RECOVERY_OUTCOME_ACTION_COMPLETION_RUN, recovery_outcome_action_completion_run)

    if not doc.get("recovery_outcome_action_activation_run"):
        frappe.throw(_("Recovery Outcome Action Activation Run is required."))

    source = frappe.get_doc(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, doc.recovery_outcome_action_activation_run)
    created_count = _generate_lines_from_action_activation_run(doc, source)

    if created_count == 0 and not doc.get("recovery_outcome_action_completion_lines"):
        doc.no_recovery_outcome_action_completion_candidate_found = 1
    else:
        doc.no_recovery_outcome_action_completion_candidate_found = 0
    ...
    return {
        ...
        "summary": summary,
        "message": f"Recovery outcome action completion lines generated. Created: {created_count}.",
    }


@frappe.whitelist()
def validate_recovery_outcome_action_completion_readiness(recovery_outcome_action_completion_run):
    _check_role()
    ...
    return {
        "summary": summary,
        "message": "Recovery Outcome Action Completion Run readiness validated.",
    }


@frappe.whitelist()
def mark_ready_for_recovery_outcome_action_completion(recovery_outcome_action_completion_run):
    _check_role()

    doc = frappe.get_doc(RECOVERY_OUTCOME_ACTION_COMPLETION_RUN, recovery_outcome_action_completion_run)
    summary = _calculate_readiness(doc)

    if not summary["recovery_outcome_action_completion_run_ready"]:
        frappe.throw(
            _("Cannot mark Ready for Action Completion. Incomplete items: {0}").format(
                "; ".join(summary["incomplete"])
            )
        )

    doc.status = "Ready for Action Completion"
    doc.readiness_percent = summary["readiness_percent"]
    doc.recovery_outcome_action_completion_run_ready = 1
    doc.save(ignore_permissions=True)
    ...
    return {
        "doctype": RECOVERY_OUTCOME_ACTION_COMPLETION_RUN,
        "name": doc.name,
        "message": "Recovery Outcome Action Completion Run marked Ready.",
    }


@frappe.whitelist()
def approve_recovery_outcome_action_completion_run(recovery_outcome_action_completion_run):
    _check_role()
    ...
    if not summary["recovery_outcome_action_completion_run_ready"]:
        frappe.throw(
            _("Cannot approve Recovery Outcome Action Completion Run. Incomplete items: {0}").format(
                "; ".join(summary["incomplete"])
            )
        )

    doc.status = "Action Completion Approved"
    doc.readiness_percent = summary["readiness_percent"]
    doc.recovery_outcome_action_completion_run_ready = 1
    ...
    for row in doc.get("recovery_outcome_action_completion_lines") or []:
        if row.get("recovery_outcome_action_completion_line_status") in ["Draft", "Ready"]:
            row.recovery_outcome_action_completion_line_status = "Approved"
    ...
    return {
        "doctype": RECOVERY_OUTCOME_ACTION_COMPLETION_RUN,
        "name": doc.name,
        "message": "Recovery Outcome Action Completion Run approved. No action was completed yet.",
    }
...
def _ready_lines_for_completion(doc):
    ...
    for row in _active_lines(doc):
        if row.get("recovery_outcome_action_completion_line_status") not in ["Approved", "Ready"]:
            continue

        if not row.get("line_ready_for_recovery_outcome_action_completion"):
            continue

        if not row.get("recovery_outcome_action_completion_source_ready"):
            continue

        if not row.get("action_completion_review_complete"):
            continue

        if not row.get("action_completion_authorized"):
            continue

        if _target_mode_needs_todo(mode) and not row.get("todo_completion_authorized"):
            continue

        if _target_mode_needs_task(mode) and not row.get("task_completion_authorized"):
            continue
...
def _complete_todo(run_doc, row):
    todo_name = row.get("target_todo")

    if not todo_name:
        frappe.throw(_("Target ToDo is required for ToDo completion mode."))

    if not _target_doc_is_phase49_released(
        TODO,
        todo_name,
        run_doc.get("recovery_outcome_action_draft_run"),
        run_doc.get("recovery_outcome_action_activation_run"),
    ):
        frappe.throw(_("ToDo {0} was not released by the linked Phase 49 run.").format(todo_name))

    todo = frappe.get_doc(TODO, todo_name)

    _set_first_existing(todo, ["ndis_crm_recovery_outcome_action_completion_run"], run_doc.name)
    _set_first_existing(todo, ["ndis_crm_recovery_outcome_action_completion_line"], row.name)
    _set_first_existing(todo, ["ndis_crm_action_completion_status"], "Completed")
    _set_first_existing(todo, ["ndis_crm_action_completion_blocked"], 0)
    _set_first_existing(todo, ["ndis_crm_action_activation_blocked"], 0)
    _set_first_existing(todo, ["ndis_crm_action_draft_blocked"], 0)

    if _field_exists(TODO, "status"):
        todo.status = "Closed"
...
def _complete_task(run_doc, row):
    task_name = row.get("target_task")

    if not task_name:
        frappe.throw(_("Target Task is required for Task completion mode."))
    ...
    if not _target_doc_is_phase49_released(
        ...
        frappe.throw(_("Task {0} was not released by the linked Phase 49 run.").format(task_name))

    task = frappe.get_doc(TASK, task_name)

    _set_first_existing(task, ["ndis_crm_recovery_outcome_action_completion_run"], run_doc.name)
    _set_first_existing(task, ["ndis_crm_recovery_outcome_action_completion_line"], row.name)
    _set_first_existing(task, ["ndis_crm_action_completion_status"], "Completed")
    _set_first_existing(task, ["ndis_crm_action_completion_blocked"], 0)
    _set_first_existing(task, ["ndis_crm_action_activation_blocked"], 0)
    _set_first_existing(task, ["ndis_crm_action_draft_blocked"], 0)

    if _field_exists(TASK, "status"):
...
@frappe.whitelist()
def complete_recovery_outcome_actions(recovery_outcome_action_completion_run):
    _check_role()

    doc = frappe.get_doc(RECOVERY_OUTCOME_ACTION_COMPLETION_RUN, recovery_outcome_action_completion_run)

    if doc.status != "Action Completion Approved":
        frappe.throw(_("Recovery Outcome Action Completion Run must be approved before completing actions."))

    if not doc.get("action_completion_allowed"):
        frappe.throw(_("Tick Action Completion Allowed before completing actions."))

    mode = doc.get("target_action_completion_mode") or "CRM Action Completion Only"

    valid_modes = [
        "CRM Action Completion Only",
        "Complete Released ToDo If Available",
        "Complete Released Task If Available",
        "Complete Released ToDo and Task If Available",
    ]

    if mode not in valid_modes:
        frappe.throw(_("Unsupported Target Action Completion Mode: {0}").format(mode))

    if _target_mode_needs_todo(mode):
        if not doc.get("todo_completion_allowed"):
            frappe.throw(_("Tick ToDo Completion Allowed for ToDo completion mode."))
        if not _doctype_exists(TODO):
            frappe.throw(_("ToDo DocType is not available."))

    if _target_mode_needs_task(mode):
        if not doc.get("task_completion_allowed"):
            frappe.throw(_("Tick Task Completion Allowed for Task completion mode."))
    ...
    for label, fieldname in blocked_run_fields:
        if doc.get(fieldname):
            frappe.throw(_("{0} must remain unticked in Phase 50.").format(label))

    summary = _calculate_readiness(doc)

    if not summary["recovery_outcome_action_completion_run_ready"]:
        frappe.throw(
            _("Cannot complete recovery outcome actions. Incomplete items: {0}").format(
                "; ".join(summary["incomplete"])
            )
        )
    ...
    if not ready_lines and not doc.get("no_recovery_outcome_action_completion_candidate_found"):
        frappe.throw(_("No ready recovery outcome action completion lines found."))
    ...
    for row in ready_lines:
        row.target_completion_mode_snapshot = mode

        if mode == "CRM Action Completion Only":
            row.recovery_outcome_action_completion_line_status = "CRM Action Completed"
            row.source_target_action_status = "CRM Action Completed"
            crm_completed.append(row.name)

        elif mode == "Complete Released ToDo If Available":
            todo_name = _complete_todo(doc, row)
            row.target_todo = todo_name
            row.target_todo_status = "Completed"
            row.recovery_outcome_action_completion_line_status = "Released ToDo Completed"
            todo_completed.append(todo_name)

        elif mode == "Complete Released Task If Available":
            task_name = _complete_task(doc, row)
            row.target_task = task_name
            row.target_task_status = "Completed"
            row.recovery_outcome_action_completion_line_status = "Released Task Completed"
            task_completed.append(task_name)

        elif mode == "Complete Released ToDo and Task If Available":
            todo_name = _complete_todo(doc, row)
            task_name = _complete_task(doc, row)
            row.target_todo = todo_name
            row.target_todo_status = "Completed"
            row.target_task = task_name
            row.target_task_status = "Completed"
            row.recovery_outcome_action_completion_line_status = "Released ToDo and Task Completed"
            todo_completed.append(todo_name)
            task_completed.append(task_name)

    doc.status = "Actions Completed"
    doc.action_completion_allowed = 0
    doc.todo_completion_allowed = 0
    doc.task_completion_allowed = 0
    doc.communication_creation_allowed = 0
    ...
    return {
        "crm_completed": crm_completed,
        "todo_completed": todo_completed,
        "task_completed": task_completed,
        "message": "Recovery outcome actions completed. No Communication, Email Queue, Event, Recovery Case, Journal Entry, GL Entry, Payment Entry, Sales Invoice, adjustment, bank reconciliation, Claim Batch, Claim Line, or Remittance Import was created.",
    }


def validate_recovery_outcome_action_completion_run(doc, method=None):
    summary = _calculate_readiness(doc)
    _sync_totals(doc)
    ...
    if doc.status in READY_STATUSES and not summary["recovery_outcome_action_completion_run_ready"]:
        frappe.throw(
            _("Cannot set Recovery Outcome Action Completion Run to {0}. Incomplete items: {1}").format(
                doc.status,
                "; ".join(summary["incomplete"])
            )
        )

    if doc.status != "Action Completion Approved" and doc.get("action_completion_allowed"):
        frappe.throw(_("Action Completion Allowed can only be ticked after the run is approved."))

    mode = doc.get("target_action_completion_mode") or "CRM Action Completion Only"

    if doc.get("todo_completion_allowed") and not _target_mode_needs_todo(mode):
        frappe.throw(_("ToDo Completion Allowed can only be used with a ToDo completion mode."))

    if doc.get("task_completion_allowed") and not _target_mode_needs_task(mode):
        frappe.throw(_("Task Completion Allowed can only be used with a Task completion mode."))
    ...
    for label, fieldname in blocked_run_fields:
        if doc.get(fieldname):
            frappe.throw(_("{0} is not allowed in Phase 50.").format(label))


def on_recovery_outcome_action_completion_run_update(doc, method=None):
    try:
        _sync_summary_to_links(doc)
    except Exception:
        frappe.log_error(
            frappe.get_traceback(),
            "NDIS CRM Recovery Outcome Action Completion Run Summary Sync Failed"
        )


def validate_optional_phase50_todo_guard(doc, method=None):
    if not _field_exists(doc.doctype, "ndis_crm_recovery_outcome_action_draft_run"):
        return

    if not doc.get("ndis_crm_recovery_outcome_action_draft_run"):
        return
    ...
    if _field_exists(doc.doctype, "ndis_crm_action_completion_blocked") and doc.get("ndis_crm_action_completion_blocked"):
        if _field_exists(doc.doctype, "status") and doc.get("status") in ["Closed", "Cancelled", "Completed"]:
            frappe.throw(_("This ToDo is still blocked from completion until the controlled Phase 50 completion gate releases it."))


def validate_optional_phase50_task_guard(doc, method=None):
    if not _field_exists(doc.doctype, "ndis_crm_recovery_outcome_action_draft_run"):
        return
    ...
    if _field_exists(doc.doctype, "ndis_crm_action_completion_blocked") and doc.get("ndis_crm_action_completion_blocked"):
        if _field_exists(doc.doctype, "status") and doc.get("status") in ["Completed", "Cancelled", "Closed"]:
            frappe.throw(_("This Task is still blocked from completion until the controlled Phase 50 completion gate releases it."))


def validate_crm_deal_phase50(doc, method=None):
    if doc.status != "Won / Active Client":
        return

    required = 0

    if _field_exists(CRM_DEAL, "ndis_recovery_outcome_action_completion_run_required"):
        required = doc.get("ndis_recovery_outcome_action_completion_run_required")

    if not required:
        return

    run = doc.get("ndis_recovery_outcome_action_completion_run") if _field_exists(CRM_DEAL, "ndis_recovery_outcome_action_completion_run") else None

    if not run:
        frappe.throw(
            _("Cannot mark CRM Deal as Won / Active Client. NDIS Recovery Outcome Action Completion Run must be created and approved/completed.")
        )

    if not _is_action_completion_approved(run):
        frappe.throw(
            _("Cannot mark CRM Deal as Won / Active Client. NDIS Recovery Outcome Action Completion Run must be approved/completed.")
        )


def validate_crm_deal_phase50_combined(doc, method=None):
    """
    Preserve Phase 2-49 validator chain, then add optional Phase 50 Recovery Outcome Action Completion validation.
    """
    try:
        ...
        try:
            from ndis_crm.phase48_recovery_outcome_action_draft import validate_crm_deal_phase48_combined
            validate_crm_deal_phase48_combined(doc, method)
        except ImportError:
            pass

    validate_crm_deal_phase50(doc, method)


def phase50_health_check():
    print("---- NDIS CRM Phase 50 Health Check ----")

    for dt in [
        RECOVERY_OUTCOME_ACTION_COMPLETION_LINE,
        RECOVERY_OUTCOME_ACTION_COMPLETION_RUN,
        RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN,
        RECOVERY_OUTCOME_ACTION_ACTIVATION_LINE,
    ...
    if _doctype_exists(TODO):
        for field in [
            "ndis_crm_recovery_outcome_action_completion_run",
            "ndis_crm_recovery_outcome_action_completion_line",
            "ndis_crm_action_completion_status",
        ]:
    ...
    print(
        "NDIS CRM Recovery Outcome Action Completion Run records:",
        frappe.db.count(RECOVERY_OUTCOME_ACTION_COMPLETION_RUN) if _doctype_exists(RECOVERY_OUTCOME_ACTION_COMPLETION_RUN) else 0
    )
    print("Phase 50 creates CRM outcome action completion run/line records only.")
    print("Phase 50 can optionally complete existing Phase 49-released ToDo/Task records only when explicitly allowed.")
    print("Phase 50 does not create ToDo, Task, Communication, Email Queue, Event, Recovery Case, Journal Entry, GL Entry, Payment Entry, Sales Invoice, adjustment, bank reconciliation, Claim Batch, Claim Line, or NDIS Remittance Import.")
    print("---- End Phase 50 Health Check ----")
