...
def _check_role():
    if not set(frappe.get_roles()).intersection(ALLOWED_ROLES):
        frappe.throw(_("You do not have permission to perform this recovery outcome action activation action."))
...
def _target_mode_needs_todo(mode):
    return mode in ["Release Draft ToDo If Available", "Release Draft ToDo and Task If Available"]


def _target_mode_needs_task(mode):
    return mode in ["Release Draft Task If Available", "Release Draft ToDo and Task If Available"]


def _existing_run_for_action_draft_run(recovery_outcome_action_draft_run):
    if not _doctype_exists(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN):
        return None

    if _field_exists(RECOVERY_OUTCOME_ACTION_DRAFT_RUN, "ndis_recovery_outcome_action_activation_run"):
    ...
    return frappe.db.get_value(
        ...
        {"recovery_outcome_action_draft_run": recovery_outcome_action_draft_run},
        "name",
    )
...
def _existing_run_for_deal(deal):
    ...
    if _field_exists(CRM_DEAL, "ndis_recovery_outcome_action_activation_run"):
        existing = frappe.db.get_value(CRM_DEAL, deal, "ndis_recovery_outcome_action_activation_run")
        if existing:
            return existing
...
def _get_action_draft_run_for_deal(deal):
    if _field_exists(CRM_DEAL, "ndis_recovery_outcome_action_draft_run"):
        run = frappe.db.get_value(CRM_DEAL, deal, "ndis_recovery_outcome_action_draft_run")
        if run:
            return run
...
def _is_action_draft_completed(run):
    ...
    return status == "Action Drafts Created" and bool(ready)


def _is_action_activation_approved(run):
...
def _append_line_if_missing(doc, row_data):
    existing = {
        row.recovery_outcome_action_activation_source_key
        for row in doc.get("recovery_outcome_action_activation_lines") or []
        if row.get("recovery_outcome_action_activation_source_key")
    }

    key = row_data.get("recovery_outcome_action_activation_source_key")

    if key and key in existing:
        return False

    doc.append("recovery_outcome_action_activation_lines", row_data)
    return True


def _source_key(row):
    return row.get("recovery_outcome_action_draft_source_key") or "|".join([
        str(row.get("target_todo") or ""),
        str(row.get("target_task") or ""),
        str(row.get("target_action_name") or ""),
        str(row.get("action_type") or ""),
        str(row.get("service_line") or ""),
    ])


def _is_action_draft_line_done(row):
    return row.get("recovery_outcome_action_draft_line_status") in [
        "CRM Action Draft Prepared",
        "Draft ToDo Created",
        "Draft Task Created",
        "Draft ToDo and Task Created",
    ]


def _target_doc_is_phase48_marked(doctype, name, draft_run):
    if not doctype or not name or not draft_run:
        return False
    ...
    return frappe.db.get_value(doctype, name, "ndis_crm_recovery_outcome_action_draft_run") == draft_run


def _build_activation_line(row, source_doc):
    source_ready = bool(
        source_doc.get("status") == "Action Drafts Created"
        and _is_action_draft_line_done(row)
        and row.get("recovery_outcome_action_draft_source_ready")
        and row.get("action_draft_review_complete")
        and row.get("action_draft_authorized")
        and row.get("line_ready_for_recovery_outcome_action_draft")
        and not row.get("todo_activation_authorized")
        and not row.get("task_activation_authorized")
        and not row.get("task_completion_authorized")
        ...
        and not row.get("recovery_outcome_action_draft_hold")
        and _to_float(row.get("proposed_recovery_amount")) > 0
    )

    return {
        "recovery_outcome_action_activation_source_key": _source_key(row),

        "source_target_action_mode_snapshot": row.get("target_action_mode_snapshot"),
        "source_target_action_doctype": row.get("target_action_doctype"),
        "source_target_action_name": row.get("target_action_name"),
        "source_target_action_status": row.get("target_action_status"),
        "target_activation_mode_snapshot": None,
        ...
        "recovery_outcome_action_activation_source_ready": 1 if source_ready else 0,
        "action_type": row.get("action_type"),
        "action_route": row.get("action_route"),
        "action_priority": row.get("action_priority") or "Normal",
        "action_owner": row.get("action_owner") or source_doc.get("recovery_outcome_action_draft_owner") or frappe.session.user,
        ...
        "action_activation_review_complete": 0,
        "action_activation_authorized": 0,
        "todo_release_authorized": 0,
        "task_release_authorized": 0,
        ...
        "recovery_outcome_action_activation_hold": 0 if source_ready else 1,
        "recovery_outcome_action_activation_hold_reason": None if source_ready else "Phase 48 action draft source is not ready for activation.",
        "line_ready_for_recovery_outcome_action_activation": 0,
        "recovery_outcome_action_activation_line_status": "Draft",
        "notes": row.get("notes"),
    }


def _generate_lines_from_action_draft_run(doc, source):
    created = 0

    for row in source.get("recovery_outcome_action_draft_lines") or []:
        if not _is_action_draft_line_done(row):
            continue

        if _to_float(row.get("proposed_recovery_amount")) <= 0:
            continue

        if _append_line_if_missing(doc, _build_activation_line(row, source)):
            created += 1
...
def _active_lines(doc):
    return [
        row for row in doc.get("recovery_outcome_action_activation_lines") or []
        if _to_float(row.get("proposed_recovery_amount")) > 0
    ]
...
def _calculate_totals(doc):
    totals = {
        "recovery_outcome_action_activation_line_count": 0,
        "recovery_outcome_action_activation_amount_total": 0,
        "recovery_outcome_action_activation_ready_count": 0,
        "recovery_outcome_action_activation_hold_count": 0,
        "crm_action_activated_count": 0,
        "todo_released_count": 0,
        "task_released_count": 0,
        "action_activated_count": 0,
        "blocked_action_completion_count": 0,
    ...
    for row in doc.get("recovery_outcome_action_activation_lines") or []:
        totals["recovery_outcome_action_activation_line_count"] += 1
        totals["recovery_outcome_action_activation_amount_total"] += _to_float(row.get("proposed_recovery_amount"))

        if row.get("line_ready_for_recovery_outcome_action_activation"):
            totals["recovery_outcome_action_activation_ready_count"] += 1

        if row.get("recovery_outcome_action_activation_hold"):
            totals["recovery_outcome_action_activation_hold_count"] += 1

        status = row.get("recovery_outcome_action_activation_line_status")
        if status == "CRM Action Activated":
            totals["crm_action_activated_count"] += 1
            totals["action_activated_count"] += 1
        elif status == "Draft ToDo Released":
            totals["todo_released_count"] += 1
            totals["action_activated_count"] += 1
        elif status == "Draft Task Released":
            totals["task_released_count"] += 1
            totals["action_activated_count"] += 1
        elif status == "Draft ToDo and Task Released":
            totals["todo_released_count"] += 1
            totals["task_released_count"] += 1
            totals["action_activated_count"] += 1

        blocked_fields = [
            ("blocked_action_completion_count", "action_completion_authorized"),
    ...
    totals["recovery_outcome_action_activation_amount_total"] = round(
        totals["recovery_outcome_action_activation_amount_total"],
        2,
    )
...
def _sync_totals(doc):
    ...
    for fieldname, value in totals.items():
        if _field_exists(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, fieldname):
            doc.set(fieldname, value)
...
def _calculate_readiness(doc):
    ...
    checks.append({
        "label": "Recovery Outcome Action Draft Run linked",
        "complete": bool(doc.get("recovery_outcome_action_draft_run")),
    })

    checks.append({
        "label": "Recovery outcome action drafts created",
        "complete": _is_action_draft_completed(doc.get("recovery_outcome_action_draft_run")),
    })
    ...
    checks.append({
        "label": "Recovery Outcome Action Activation Owner assigned",
        "complete": bool(doc.get("recovery_outcome_action_activation_owner")),
    })

    mode = doc.get("target_action_activation_mode") or "CRM Action Activation Only"

    if _target_mode_needs_todo(mode):
        checks.append({
            "label": "ToDo DocType exists for ToDo release mode",
            "complete": _doctype_exists(TODO),
        })

    if _target_mode_needs_task(mode):
        checks.append({
            "label": "Task DocType exists for Task release mode",
            "complete": _doctype_exists(TASK),
        })

    lines = _active_lines(doc)
    no_candidates = bool(doc.get("no_recovery_outcome_action_activation_candidate_found"))

    checks.append({
        "label": "Action activation lines exist or no-candidate flag is set",
        "complete": bool(lines) or no_candidates,
    })

    if lines:
        source_not_ready = [row.service_line for row in lines if not row.get("recovery_outcome_action_activation_source_ready")]
        checks.append({
            "label": "Action activation source-ready flags are complete",
        ...
        review_missing = [row.service_line for row in lines if not row.get("action_activation_review_complete")]
        checks.append({
            "label": "Action activation review complete",
            "complete": not review_missing,
            "details": review_missing,
        })

        auth_missing = [row.service_line for row in lines if not row.get("action_activation_authorized")]
        checks.append({
            "label": "Action activation authorized",
        ...
        if _target_mode_needs_todo(mode):
            ...
            checks.append({
                "label": "Existing Phase 48 ToDo draft linked for all ToDo release lines",
                "complete": not missing_todo,
                "details": missing_todo,
            })

            todo_auth_missing = [row.service_line for row in lines if not row.get("todo_release_authorized")]
            checks.append({
                "label": "ToDo release authorized for all ToDo release lines",
        ...
        if _target_mode_needs_task(mode):
            ...
            checks.append({
                "label": "Existing Phase 48 Task draft linked for all Task release lines",
                "complete": not missing_task,
                "details": missing_task,
            })

            task_auth_missing = [row.service_line for row in lines if not row.get("task_release_authorized")]
            checks.append({
                "label": "Task release authorized for all Task release lines",
                "complete": not task_auth_missing,
                "details": task_auth_missing,
            })

        blocked_fields = [
            ("Action completion remains blocked in Phase 49", "action_completion_authorized"),
            ("Communication creation remains blocked in Phase 49", "communication_creation_authorized"),
            ("Email sending remains blocked in Phase 49", "email_send_authorized"),
            ("Event creation remains blocked in Phase 49", "event_creation_authorized"),
            ("Recovery Case creation remains blocked in Phase 49", "recovery_case_creation_authorized"),
            ("Journal Entry authorization remains blocked in Phase 49", "journal_entry_authorized"),
            ("Manual GL authorization remains blocked in Phase 49", "manual_gl_authorized"),
            ("Payment Entry authorization remains blocked in Phase 49", "payment_entry_authorized"),
            ("Sales Invoice authorization remains blocked in Phase 49", "sales_invoice_authorized"),
            ("Adjustment authorization remains blocked in Phase 49", "adjustment_authorized"),
            ("Bank reconciliation authorization remains blocked in Phase 49", "bank_reconciliation_authorized"),
            ("Claim Batch authorization remains blocked in Phase 49", "claim_batch_authorized"),
            ("Claim Line authorization remains blocked in Phase 49", "claim_line_authorized"),
            ("Remittance Import authorization remains blocked in Phase 49", "remittance_import_authorized"),
        ]
        ...
        holds = [row.service_line for row in lines if row.get("recovery_outcome_action_activation_hold")]
        checks.append({
            "label": "No active action activation hold remains",
            "complete": not holds,
            "details": holds,
        })

        not_ready = [row.service_line for row in lines if not row.get("line_ready_for_recovery_outcome_action_activation")]
        checks.append({
            "label": "All active lines marked ready for action activation",
    ...
    return {
        ...
        "recovery_outcome_action_activation_run_ready": total > 0 and complete == total,
...
def _sync_summary_to_links(doc):
    ...
    if _field_exists(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, "readiness_percent"):
        doc.readiness_percent = summary["readiness_percent"]

    if _field_exists(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, "recovery_outcome_action_activation_run_ready"):
        doc.recovery_outcome_action_activation_run_ready = 1 if summary["recovery_outcome_action_activation_run_ready"] else 0
    ...
    for doctype, name in targets:
        ...
        _db_set_if_field(doctype, name, "ndis_recovery_outcome_action_activation_run", doc.name)
        _db_set_if_field(doctype, name, "recovery_outcome_action_activation_status", doc.status)
        _db_set_if_field(doctype, name, "recovery_outcome_action_activation_ready", 1 if summary["recovery_outcome_action_activation_run_ready"] else 0)
...
@frappe.whitelist()
def create_recovery_outcome_action_activation_run_from_action_draft_run(recovery_outcome_action_draft_run):
    _check_role()

    if not recovery_outcome_action_draft_run:
        frappe.throw(_("NDIS CRM Recovery Outcome Action Draft Run is required."))

    if not frappe.db.exists(RECOVERY_OUTCOME_ACTION_DRAFT_RUN, recovery_outcome_action_draft_run):
        frappe.throw(_("NDIS CRM Recovery Outcome Action Draft Run {0} was not found.").format(recovery_outcome_action_draft_run))

    existing = _existing_run_for_action_draft_run(recovery_outcome_action_draft_run)
    if existing:
        return {
            "doctype": RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN,
            "name": existing,
            "created": False,
            "message": "Existing NDIS CRM Recovery Outcome Action Activation Run returned.",
        }

    source = frappe.get_doc(RECOVERY_OUTCOME_ACTION_DRAFT_RUN, recovery_outcome_action_draft_run)

    doc = frappe.new_doc(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN)
    doc.status = "Draft"
    doc.recovery_outcome_action_draft_run = source.name
    ...
    doc.recovery_outcome_action_activation_owner = frappe.session.user
    doc.recovery_outcome_action_draft_owner = source.get("recovery_outcome_action_draft_owner")
    doc.target_action_activation_mode = "CRM Action Activation Only"
    ...
    doc.todo_release_allowed = 0
    doc.task_release_allowed = 0
    ...
    created_count = _generate_lines_from_action_draft_run(doc, source)
    doc.no_recovery_outcome_action_activation_candidate_found = 1 if created_count == 0 else 0
    ...
    return {
        ...
        "name": doc.name,
        "created": True,
        "recovery_outcome_action_activation_line_count": created_count,
        "no_recovery_outcome_action_activation_candidate_found": bool(doc.no_recovery_outcome_action_activation_candidate_found),
        "message": "NDIS CRM Recovery Outcome Action Activation Run created successfully.",
    }


@frappe.whitelist()
def create_recovery_outcome_action_activation_run_from_crm_deal(deal):
    _check_role()
    ...
    source_run = _get_action_draft_run_for_deal(deal)

    if not source_run:
        frappe.throw(_("Please complete NDIS CRM Recovery Outcome Action Draft Run before creating Recovery Outcome Action Activation Run."))

    return create_recovery_outcome_action_activation_run_from_action_draft_run(source_run)


@frappe.whitelist()
def generate_recovery_outcome_action_activation_lines(recovery_outcome_action_activation_run):
    _check_role()

    doc = frappe.get_doc(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, recovery_outcome_action_activation_run)

    if not doc.get("recovery_outcome_action_draft_run"):
        frappe.throw(_("Recovery Outcome Action Draft Run is required."))

    source = frappe.get_doc(RECOVERY_OUTCOME_ACTION_DRAFT_RUN, doc.recovery_outcome_action_draft_run)
    created_count = _generate_lines_from_action_draft_run(doc, source)

    if created_count == 0 and not doc.get("recovery_outcome_action_activation_lines"):
        doc.no_recovery_outcome_action_activation_candidate_found = 1
    else:
        doc.no_recovery_outcome_action_activation_candidate_found = 0
    ...
    return {
        ...
        "summary": summary,
        "message": f"Recovery outcome action activation lines generated. Created: {created_count}.",
    }


@frappe.whitelist()
def validate_recovery_outcome_action_activation_readiness(recovery_outcome_action_activation_run):
    _check_role()
    ...
    return {
        "summary": summary,
        "message": "Recovery Outcome Action Activation Run readiness validated.",
    }


@frappe.whitelist()
def mark_ready_for_recovery_outcome_action_activation(recovery_outcome_action_activation_run):
    _check_role()

    doc = frappe.get_doc(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, recovery_outcome_action_activation_run)
    summary = _calculate_readiness(doc)

    if not summary["recovery_outcome_action_activation_run_ready"]:
        frappe.throw(
            _("Cannot mark Ready for Action Activation. Incomplete items: {0}").format(
                "; ".join(summary["incomplete"])
            )
        )

    doc.status = "Ready for Action Activation"
    doc.readiness_percent = summary["readiness_percent"]
    doc.recovery_outcome_action_activation_run_ready = 1
    doc.save(ignore_permissions=True)
    ...
    return {
        "doctype": RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN,
        "name": doc.name,
        "message": "Recovery Outcome Action Activation Run marked Ready.",
    }


@frappe.whitelist()
def approve_recovery_outcome_action_activation_run(recovery_outcome_action_activation_run):
    _check_role()
    ...
    if not summary["recovery_outcome_action_activation_run_ready"]:
        frappe.throw(
            _("Cannot approve Recovery Outcome Action Activation Run. Incomplete items: {0}").format(
                "; ".join(summary["incomplete"])
            )
        )

    doc.status = "Action Activation Approved"
    doc.readiness_percent = summary["readiness_percent"]
    doc.recovery_outcome_action_activation_run_ready = 1
    ...
    for row in doc.get("recovery_outcome_action_activation_lines") or []:
        if row.get("recovery_outcome_action_activation_line_status") in ["Draft", "Ready"]:
            row.recovery_outcome_action_activation_line_status = "Approved"
    ...
    return {
        "doctype": RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN,
        "name": doc.name,
        "message": "Recovery Outcome Action Activation Run approved. No action was activated yet.",
    }
...
def _ready_lines_for_activation(doc):
    ...
    for row in _active_lines(doc):
        if row.get("recovery_outcome_action_activation_line_status") not in ["Approved", "Ready"]:
            continue

        if not row.get("line_ready_for_recovery_outcome_action_activation"):
            continue

        if not row.get("recovery_outcome_action_activation_source_ready"):
            continue

        if not row.get("action_activation_review_complete"):
            continue

        if not row.get("action_activation_authorized"):
            continue

        if _target_mode_needs_todo(mode) and not row.get("todo_release_authorized"):
            continue

        if _target_mode_needs_task(mode) and not row.get("task_release_authorized"):
            continue

        blocked = any([
            row.get("action_completion_authorized"),
...
def _release_todo(run_doc, row):
    todo_name = row.get("target_todo")

    if not todo_name:
        frappe.throw(_("Target ToDo is required for ToDo release mode."))

    if not _target_doc_is_phase48_marked(TODO, todo_name, run_doc.get("recovery_outcome_action_draft_run")):
        frappe.throw(_("ToDo {0} was not created by the linked Phase 48 run.").format(todo_name))

    todo = frappe.get_doc(TODO, todo_name)

    _set_first_existing(todo, ["ndis_crm_recovery_outcome_action_activation_run"], run_doc.name)
    _set_first_existing(todo, ["ndis_crm_recovery_outcome_action_activation_line"], row.name)
    _set_first_existing(todo, ["ndis_crm_action_activation_status"], "Released / Active")
    ...
    if _field_exists(TODO, "status") and todo.get("status") not in ["Open"]:
        todo.status = "Open"
...
def _release_task(run_doc, row):
    task_name = row.get("target_task")

    if not task_name:
        frappe.throw(_("Target Task is required for Task release mode."))

    if not _target_doc_is_phase48_marked(TASK, task_name, run_doc.get("recovery_outcome_action_draft_run")):
        frappe.throw(_("Task {0} was not created by the linked Phase 48 run.").format(task_name))

    task = frappe.get_doc(TASK, task_name)

    _set_first_existing(task, ["ndis_crm_recovery_outcome_action_activation_run"], run_doc.name)
    _set_first_existing(task, ["ndis_crm_recovery_outcome_action_activation_line"], row.name)
    _set_first_existing(task, ["ndis_crm_action_activation_status"], "Released / Active")
    ...
    if _field_exists(TASK, "status") and task.get("status") not in ["Open"]:
        task.status = "Open"
...
@frappe.whitelist()
def activate_recovery_outcome_actions(recovery_outcome_action_activation_run):
    _check_role()

    doc = frappe.get_doc(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN, recovery_outcome_action_activation_run)

    if doc.status != "Action Activation Approved":
        frappe.throw(_("Recovery Outcome Action Activation Run must be approved before activating actions."))

    if not doc.get("action_activation_allowed"):
        frappe.throw(_("Tick Action Activation Allowed before activating actions."))

    mode = doc.get("target_action_activation_mode") or "CRM Action Activation Only"

    valid_modes = [
        "CRM Action Activation Only",
        "Release Draft ToDo If Available",
        "Release Draft Task If Available",
        "Release Draft ToDo and Task If Available",
    ]

    if mode not in valid_modes:
        frappe.throw(_("Unsupported Target Action Activation Mode: {0}").format(mode))

    if _target_mode_needs_todo(mode):
        if not doc.get("todo_release_allowed"):
            frappe.throw(_("Tick ToDo Release Allowed for ToDo release mode."))
        if not _doctype_exists(TODO):
            frappe.throw(_("ToDo DocType is not available."))

    if _target_mode_needs_task(mode):
        if not doc.get("task_release_allowed"):
            frappe.throw(_("Tick Task Release Allowed for Task release mode."))
    ...
    for label, fieldname in blocked_run_fields:
        if doc.get(fieldname):
            frappe.throw(_("{0} must remain unticked in Phase 49.").format(label))

    summary = _calculate_readiness(doc)

    if not summary["recovery_outcome_action_activation_run_ready"]:
        frappe.throw(
            _("Cannot activate recovery outcome actions. Incomplete items: {0}").format(
                "; ".join(summary["incomplete"])
            )
        )
    ...
    if not ready_lines and not doc.get("no_recovery_outcome_action_activation_candidate_found"):
        frappe.throw(_("No ready recovery outcome action activation lines found."))
    ...
    for row in ready_lines:
        row.target_activation_mode_snapshot = mode

        if mode == "CRM Action Activation Only":
            row.recovery_outcome_action_activation_line_status = "CRM Action Activated"
            row.source_target_action_status = "CRM Action Activated"
            crm_activated.append(row.name)

        elif mode == "Release Draft ToDo If Available":
            todo_name = _release_todo(doc, row)
            row.target_todo = todo_name
            row.target_todo_status = "Released / Active"
            row.recovery_outcome_action_activation_line_status = "Draft ToDo Released"
            todo_released.append(todo_name)

        elif mode == "Release Draft Task If Available":
            task_name = _release_task(doc, row)
            row.target_task = task_name
            row.target_task_status = "Released / Active"
            row.recovery_outcome_action_activation_line_status = "Draft Task Released"
            task_released.append(task_name)

        elif mode == "Release Draft ToDo and Task If Available":
            todo_name = _release_todo(doc, row)
            task_name = _release_task(doc, row)
            row.target_todo = todo_name
            row.target_todo_status = "Released / Active"
            row.target_task = task_name
            row.target_task_status = "Released / Active"
            row.recovery_outcome_action_activation_line_status = "Draft ToDo and Task Released"
            todo_released.append(todo_name)
            task_released.append(task_name)

    doc.status = "Actions Activated"
    doc.action_activation_allowed = 0
    doc.todo_release_allowed = 0
    doc.task_release_allowed = 0
    doc.action_completion_allowed = 0
    ...
    return {
        ...
        "todo_released": todo_released,
        "task_released": task_released,
        "message": "Recovery outcome actions activated/released. No action was completed. No Communication, Email Queue, Event, Recovery Case, Journal Entry, GL Entry, Payment Entry, Sales Invoice, adjustment, bank reconciliation, Claim Batch, Claim Line, or Remittance Import was created.",
    }


def validate_recovery_outcome_action_activation_run(doc, method=None):
    summary = _calculate_readiness(doc)
    _sync_totals(doc)
    ...
    if doc.status in READY_STATUSES and not summary["recovery_outcome_action_activation_run_ready"]:
        frappe.throw(
            _("Cannot set Recovery Outcome Action Activation Run to {0}. Incomplete items: {1}").format(
                doc.status,
                "; ".join(summary["incomplete"])
            )
        )

    if doc.status != "Action Activation Approved" and doc.get("action_activation_allowed"):
        frappe.throw(_("Action Activation Allowed can only be ticked after the run is approved."))

    mode = doc.get("target_action_activation_mode") or "CRM Action Activation Only"

    if doc.get("todo_release_allowed") and not _target_mode_needs_todo(mode):
        frappe.throw(_("ToDo Release Allowed can only be used with a ToDo release mode."))

    if doc.get("task_release_allowed") and not _target_mode_needs_task(mode):
        frappe.throw(_("Task Release Allowed can only be used with a Task release mode."))
    ...
    for label, fieldname in blocked_run_fields:
        if doc.get(fieldname):
            frappe.throw(_("{0} is not allowed in Phase 49.").format(label))


def on_recovery_outcome_action_activation_run_update(doc, method=None):
    try:
        _sync_summary_to_links(doc)
    except Exception:
        frappe.log_error(
            frappe.get_traceback(),
            "NDIS CRM Recovery Outcome Action Activation Run Summary Sync Failed"
        )


def validate_optional_phase49_todo_guard(doc, method=None):
    ...
    if _field_exists(doc.doctype, "ndis_crm_action_completion_blocked") and doc.get("ndis_crm_action_completion_blocked"):
        if _field_exists(doc.doctype, "status") and doc.get("status") in ["Closed", "Cancelled", "Completed"]:
            frappe.throw(_("This ToDo is still blocked from completion until a later controlled Phase 50 completion gate."))


def validate_optional_phase49_task_guard(doc, method=None):
    if not _field_exists(doc.doctype, "ndis_crm_recovery_outcome_action_draft_run"):
        return
    ...
    if _field_exists(doc.doctype, "ndis_crm_action_completion_blocked") and doc.get("ndis_crm_action_completion_blocked"):
        if _field_exists(doc.doctype, "status") and doc.get("status") in ["Completed", "Cancelled", "Closed"]:
            frappe.throw(_("This Task is still blocked from completion until a later controlled Phase 50 completion gate."))


def validate_crm_deal_phase49(doc, method=None):
    if doc.status != "Won / Active Client":
        return

    required = 0

    if _field_exists(CRM_DEAL, "ndis_recovery_outcome_action_activation_run_required"):
        required = doc.get("ndis_recovery_outcome_action_activation_run_required")

    if not required:
        return

    run = doc.get("ndis_recovery_outcome_action_activation_run") if _field_exists(CRM_DEAL, "ndis_recovery_outcome_action_activation_run") else None

    if not run:
        frappe.throw(
            _("Cannot mark CRM Deal as Won / Active Client. NDIS Recovery Outcome Action Activation Run must be created and approved/completed.")
        )

    if not _is_action_activation_approved(run):
        frappe.throw(
            _("Cannot mark CRM Deal as Won / Active Client. NDIS Recovery Outcome Action Activation Run must be approved/completed.")
        )


def validate_crm_deal_phase49_combined(doc, method=None):
    """
    Preserve Phase 2-48 validator chain, then add optional Phase 49 Recovery Outcome Action Activation validation.
    """
    try:
    ...
    validate_crm_deal_phase49(doc, method)


def phase49_health_check():
    print("---- NDIS CRM Phase 49 Health Check ----")
    ...
    if _doctype_exists(TODO):
        for field in [
            "ndis_crm_recovery_outcome_action_activation_run",
            "ndis_crm_recovery_outcome_action_activation_line",
            "ndis_crm_action_activation_status",
        ]:
    ...
    print(
        "NDIS CRM Recovery Outcome Action Activation Run records:",
        frappe.db.count(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN) if _doctype_exists(RECOVERY_OUTCOME_ACTION_ACTIVATION_RUN) else 0
    )
    print("Phase 49 creates CRM outcome action activation run/line records only.")
    print("Phase 49 can optionally release existing Phase 48 draft-marked ToDo/Task records only when explicitly allowed.")
    print("Phase 49 does not create ToDo, Task, Communication, Email Queue, Event, Recovery Case, Journal Entry, GL Entry, Payment Entry, Sales Invoice, adjustment, bank reconciliation, Claim Batch, Claim Line, or NDIS Remittance Import.")
    print("Phase 49 does not complete ToDo/Task.")
    print("---- End Phase 49 Health Check ----")
