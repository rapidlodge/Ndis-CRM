...
def _check_role():
    if not set(frappe.get_roles()).intersection(ALLOWED_ROLES):
        frappe.throw(_("You do not have permission to perform this recovery outcome closure finalisation action."))
...
def _existing_run_for_closure_draft_run(recovery_outcome_closure_draft_run):
    if not _doctype_exists(RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN):
        return None

    if _field_exists(RECOVERY_OUTCOME_CLOSURE_DRAFT_RUN, "ndis_recovery_outcome_closure_finalisation_run"):
    ...
    return frappe.db.get_value(
        ...
        {"recovery_outcome_closure_draft_run": recovery_outcome_closure_draft_run},
        "name",
    )
...
def _existing_run_for_deal(deal):
    ...
    if _field_exists(CRM_DEAL, "ndis_recovery_outcome_closure_finalisation_run"):
        existing = frappe.db.get_value(CRM_DEAL, deal, "ndis_recovery_outcome_closure_finalisation_run")
        if existing:
            return existing
...
def _get_closure_draft_run_for_deal(deal):
    if _field_exists(CRM_DEAL, "ndis_recovery_outcome_closure_draft_run"):
        run = frappe.db.get_value(CRM_DEAL, deal, "ndis_recovery_outcome_closure_draft_run")
        if run:
            return run
...
def _is_closure_draft_completed(run):
    ...
    return status == "Closure Drafts Created" and bool(ready)


def _is_closure_finalisation_approved(run):
...
def _append_line_if_missing(doc, row_data):
    existing = {
        row.recovery_outcome_closure_finalisation_source_key
        for row in doc.get("recovery_outcome_closure_finalisation_lines") or []
        if row.get("recovery_outcome_closure_finalisation_source_key")
    }

    key = row_data.get("recovery_outcome_closure_finalisation_source_key")

    if key and key in existing:
        return False

    doc.append("recovery_outcome_closure_finalisation_lines", row_data)
    return True


def _source_key(row):
    return row.get("recovery_outcome_closure_draft_source_key") or "|".join([
        str(row.get("closure_decision") or ""),
        str(row.get("target_closure_name") or ""),
        str(row.get("target_case_name") or ""),
        str(row.get("service_line") or ""),
    ])


def _is_closure_draft_line_done(row):
    return row.get("recovery_outcome_closure_draft_line_status") == "CRM Closure Draft Prepared"


def _final_outcome_from_decision(decision):
    mapping = {
        "Prepare Recovery Closure": "CRM Recovery Closed",
        "Prepare Repeat Follow-Up": "Repeat Follow-Up Required",
        "Prepare Payment Promise Monitoring Continuation": "Payment Promise Monitoring Required",
        "Prepare Evidence Follow-Up": "Evidence Follow-Up Required",
        "Prepare Dispute Review": "Dispute Review Required",
        "Prepare Contact Correction Review": "Contact Correction Review Required",
        "Prepare Escalation Review": "Escalation Review Required",
        "Prepare Manual Review": "Manual Review Required",
    }
    return mapping.get(decision or "", "Manual Review Required")


def _build_finalisation_line(row, source_doc):
    source_ready = bool(
        source_doc.get("status") == "Closure Drafts Created"
        and _is_closure_draft_line_done(row)
        and row.get("recovery_outcome_closure_draft_source_ready")
        and row.get("closure_draft_review_complete")
        and row.get("closure_draft_authorized")
        and row.get("closure_draft_creation_authorized")
        and row.get("line_ready_for_recovery_outcome_closure_draft")
        ...
        and not row.get("recovery_outcome_closure_draft_hold")
        and _to_float(row.get("proposed_recovery_amount")) > 0
    )

    final_outcome = _final_outcome_from_decision(row.get("closure_decision"))

    return {
        "recovery_outcome_closure_finalisation_source_key": _source_key(row),

        "source_closure_doctype": row.get("target_closure_doctype"),
        "source_closure_name": row.get("target_closure_name"),
        "source_closure_status": row.get("target_closure_status"),
        "target_finalisation_doctype": None,
        "target_finalisation_name": None,
        "target_finalisation_status": None,
        ...
        "closure_owner": row.get("closure_owner") or source_doc.get("recovery_outcome_closure_draft_owner") or frappe.session.user,
        ...
        "final_closure_outcome": final_outcome,
        "final_closure_summary": row.get("closure_decision_summary") or row.get("closure_instruction") or final_outcome,

        "recovery_outcome_closure_finalisation_source_ready": 1 if source_ready else 0,
        "closure_finalisation_review_complete": 0,
        "closure_finalisation_authorized": 0,
        "closure_finalisation_recorded": 0,
        ...
        "recovery_outcome_closure_finalisation_hold": 0 if source_ready else 1,
        "recovery_outcome_closure_finalisation_hold_reason": None if source_ready else "Phase 52 closure draft source is not ready for finalisation.",
        "line_ready_for_recovery_outcome_closure_finalisation": 0,
        "recovery_outcome_closure_finalisation_line_status": "Draft",
        "notes": row.get("notes"),
    }


def _generate_lines_from_closure_draft_run(doc, source):
    created = 0

    for row in source.get("recovery_outcome_closure_draft_lines") or []:
        if not _is_closure_draft_line_done(row):
            continue
...
def _active_lines(doc):
    return [
        row for row in doc.get("recovery_outcome_closure_finalisation_lines") or []
        if _to_float(row.get("proposed_recovery_amount")) > 0
    ]
...
def _calculate_totals(doc):
    totals = {
        "recovery_outcome_closure_finalisation_line_count": 0,
        "recovery_outcome_closure_finalisation_amount_total": 0,
        "recovery_outcome_closure_finalisation_ready_count": 0,
        "recovery_outcome_closure_finalisation_hold_count": 0,
        "crm_closure_finalised_count": 0,
        "crm_recovery_closed_count": 0,
        "repeat_follow_up_required_count": 0,
        "payment_promise_monitoring_required_count": 0,
        "evidence_follow_up_required_count": 0,
        "dispute_review_required_count": 0,
        "contact_correction_required_count": 0,
    ...
    for row in doc.get("recovery_outcome_closure_finalisation_lines") or []:
        totals["recovery_outcome_closure_finalisation_line_count"] += 1
        totals["recovery_outcome_closure_finalisation_amount_total"] += _to_float(row.get("proposed_recovery_amount"))

        if row.get("line_ready_for_recovery_outcome_closure_finalisation"):
            totals["recovery_outcome_closure_finalisation_ready_count"] += 1

        if row.get("recovery_outcome_closure_finalisation_hold"):
            totals["recovery_outcome_closure_finalisation_hold_count"] += 1

        if row.get("recovery_outcome_closure_finalisation_line_status") == "CRM Closure Finalised":
            totals["crm_closure_finalised_count"] += 1

        outcome = row.get("final_closure_outcome")
        if outcome == "CRM Recovery Closed":
            totals["crm_recovery_closed_count"] += 1
        elif outcome == "Repeat Follow-Up Required":
            totals["repeat_follow_up_required_count"] += 1
        elif outcome == "Payment Promise Monitoring Required":
            totals["payment_promise_monitoring_required_count"] += 1
        elif outcome == "Evidence Follow-Up Required":
            totals["evidence_follow_up_required_count"] += 1
        elif outcome == "Dispute Review Required":
            totals["dispute_review_required_count"] += 1
        elif outcome == "Contact Correction Review Required":
            totals["contact_correction_required_count"] += 1
        elif outcome == "Escalation Review Required":
            totals["escalation_review_required_count"] += 1
        elif outcome == "Manual Review Required":
            totals["manual_review_required_count"] += 1
    ...
    totals["recovery_outcome_closure_finalisation_amount_total"] = round(
        totals["recovery_outcome_closure_finalisation_amount_total"],
        2,
    )
...
def _sync_totals(doc):
    ...
    for fieldname, value in totals.items():
        if _field_exists(RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN, fieldname):
...
def _calculate_readiness(doc):
    ...
    checks.append({
        "label": "Recovery Outcome Closure Draft Run linked",
        "complete": bool(doc.get("recovery_outcome_closure_draft_run")),
    })

    checks.append({
        "label": "Recovery outcome closure drafts created",
        "complete": _is_closure_draft_completed(doc.get("recovery_outcome_closure_draft_run")),
    })
    ...
    checks.append({
        "label": "Recovery Outcome Closure Finalisation Owner assigned",
        "complete": bool(doc.get("recovery_outcome_closure_finalisation_owner")),
    })

    lines = _active_lines(doc)
    no_candidates = bool(doc.get("no_recovery_outcome_closure_finalisation_candidate_found"))

    checks.append({
        "label": "Closure finalisation lines exist or no-candidate flag is set",
        "complete": bool(lines) or no_candidates,
    })

    if lines:
        source_not_ready = [row.service_line for row in lines if not row.get("recovery_outcome_closure_finalisation_source_ready")]
        checks.append({
            "label": "Closure finalisation source-ready flags are complete",
            "complete": not source_not_ready,
            "details": source_not_ready,
        })

        missing_outcome = [row.service_line for row in lines if not row.get("final_closure_outcome")]
        checks.append({
            "label": "All lines have final closure outcome",
        ...
        review_missing = [row.service_line for row in lines if not row.get("closure_finalisation_review_complete")]
        checks.append({
            "label": "Closure finalisation review complete",
            "complete": not review_missing,
            "details": review_missing,
        })

        auth_missing = [row.service_line for row in lines if not row.get("closure_finalisation_authorized")]
        checks.append({
            "label": "Closure finalisation authorized",
            "complete": not auth_missing,
            "details": auth_missing,
        })

        recorded_missing = [row.service_line for row in lines if not row.get("closure_finalisation_recorded")]
        checks.append({
            "label": "Closure finalisation recorded",
            "complete": not recorded_missing,
            "details": recorded_missing,
        })

        blocked_fields = [
            ("Actual recovery case closure remains blocked in Phase 53", "actual_recovery_case_closure_authorized"),
            ("Communication creation remains blocked in Phase 53", "communication_creation_authorized"),
            ("Email sending remains blocked in Phase 53", "email_send_authorized"),
            ("Event creation remains blocked in Phase 53", "event_creation_authorized"),
            ("ToDo creation remains blocked in Phase 53", "todo_creation_authorized"),
            ("Task creation remains blocked in Phase 53", "task_creation_authorized"),
            ("Recovery Case creation remains blocked in Phase 53", "recovery_case_creation_authorized"),
            ("Journal Entry authorization remains blocked in Phase 53", "journal_entry_authorized"),
            ("Manual GL authorization remains blocked in Phase 53", "manual_gl_authorized"),
            ("Payment Entry authorization remains blocked in Phase 53", "payment_entry_authorized"),
            ("Sales Invoice authorization remains blocked in Phase 53", "sales_invoice_authorized"),
            ("Adjustment authorization remains blocked in Phase 53", "adjustment_authorized"),
            ("Bank reconciliation authorization remains blocked in Phase 53", "bank_reconciliation_authorized"),
            ("Claim Batch authorization remains blocked in Phase 53", "claim_batch_authorized"),
            ("Claim Line authorization remains blocked in Phase 53", "claim_line_authorized"),
            ("Remittance Import authorization remains blocked in Phase 53", "remittance_import_authorized"),
        ]
        ...
        holds = [row.service_line for row in lines if row.get("recovery_outcome_closure_finalisation_hold")]
        checks.append({
            "label": "No active closure finalisation hold remains",
            "complete": not holds,
            "details": holds,
        })

        not_ready = [row.service_line for row in lines if not row.get("line_ready_for_recovery_outcome_closure_finalisation")]
        checks.append({
            "label": "All active lines marked ready for closure finalisation",
    ...
    return {
        ...
        "recovery_outcome_closure_finalisation_run_ready": total > 0 and complete == total,
...
def _sync_summary_to_links(doc):
    ...
    if _field_exists(RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN, "readiness_percent"):
        doc.readiness_percent = summary["readiness_percent"]

    if _field_exists(RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN, "recovery_outcome_closure_finalisation_run_ready"):
        doc.recovery_outcome_closure_finalisation_run_ready = 1 if summary["recovery_outcome_closure_finalisation_run_ready"] else 0
    ...
    for doctype, name in targets:
        ...
        _db_set_if_field(doctype, name, "ndis_recovery_outcome_closure_finalisation_run", doc.name)
        _db_set_if_field(doctype, name, "recovery_outcome_closure_finalisation_status", doc.status)
        _db_set_if_field(doctype, name, "recovery_outcome_closure_finalisation_ready", 1 if summary["recovery_outcome_closure_finalisation_run_ready"] else 0)
...
@frappe.whitelist()
def create_recovery_outcome_closure_finalisation_run_from_closure_draft_run(recovery_outcome_closure_draft_run):
    _check_role()

    if not recovery_outcome_closure_draft_run:
        frappe.throw(_("NDIS CRM Recovery Outcome Closure Draft Run is required."))

    if not frappe.db.exists(RECOVERY_OUTCOME_CLOSURE_DRAFT_RUN, recovery_outcome_closure_draft_run):
        frappe.throw(_("NDIS CRM Recovery Outcome Closure Draft Run {0} was not found.").format(recovery_outcome_closure_draft_run))

    existing = _existing_run_for_closure_draft_run(recovery_outcome_closure_draft_run)
    if existing:
        return {
            "doctype": RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN,
            "name": existing,
            "created": False,
            "message": "Existing NDIS CRM Recovery Outcome Closure Finalisation Run returned.",
        }

    source = frappe.get_doc(RECOVERY_OUTCOME_CLOSURE_DRAFT_RUN, recovery_outcome_closure_draft_run)

    doc = frappe.new_doc(RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN)
    doc.status = "Draft"
    doc.recovery_outcome_closure_draft_run = source.name
    ...
    doc.recovery_outcome_closure_finalisation_owner = frappe.session.user
    doc.recovery_outcome_closure_draft_owner = source.get("recovery_outcome_closure_draft_owner")

    doc.target_closure_finalisation_mode = "CRM Closure Finalisation Only"
    doc.closure_finalisation_allowed = 0
    ...
    created_count = _generate_lines_from_closure_draft_run(doc, source)
    doc.no_recovery_outcome_closure_finalisation_candidate_found = 1 if created_count == 0 else 0
    ...
    return {
        ...
        "name": doc.name,
        "created": True,
        "recovery_outcome_closure_finalisation_line_count": created_count,
        "no_recovery_outcome_closure_finalisation_candidate_found": bool(doc.no_recovery_outcome_closure_finalisation_candidate_found),
        "message": "NDIS CRM Recovery Outcome Closure Finalisation Run created successfully.",
    }


@frappe.whitelist()
def create_recovery_outcome_closure_finalisation_run_from_crm_deal(deal):
    _check_role()
    ...
    source_run = _get_closure_draft_run_for_deal(deal)

    if not source_run:
        frappe.throw(_("Please complete NDIS CRM Recovery Outcome Closure Draft Run before creating Recovery Outcome Closure Finalisation Run."))

    return create_recovery_outcome_closure_finalisation_run_from_closure_draft_run(source_run)


@frappe.whitelist()
def generate_recovery_outcome_closure_finalisation_lines(recovery_outcome_closure_finalisation_run):
    _check_role()

    doc = frappe.get_doc(RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN, recovery_outcome_closure_finalisation_run)

    if not doc.get("recovery_outcome_closure_draft_run"):
        frappe.throw(_("Recovery Outcome Closure Draft Run is required."))

    source = frappe.get_doc(RECOVERY_OUTCOME_CLOSURE_DRAFT_RUN, doc.recovery_outcome_closure_draft_run)
    created_count = _generate_lines_from_closure_draft_run(doc, source)

    if created_count == 0 and not doc.get("recovery_outcome_closure_finalisation_lines"):
        doc.no_recovery_outcome_closure_finalisation_candidate_found = 1
    else:
        doc.no_recovery_outcome_closure_finalisation_candidate_found = 0
    ...
    return {
        ...
        "summary": summary,
        "message": f"Recovery outcome closure finalisation lines generated. Created: {created_count}.",
    }


@frappe.whitelist()
def validate_recovery_outcome_closure_finalisation_readiness(recovery_outcome_closure_finalisation_run):
    _check_role()
    ...
    return {
        "summary": summary,
        "message": "Recovery Outcome Closure Finalisation Run readiness validated.",
    }


@frappe.whitelist()
def mark_ready_for_recovery_outcome_closure_finalisation(recovery_outcome_closure_finalisation_run):
    _check_role()

    doc = frappe.get_doc(RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN, recovery_outcome_closure_finalisation_run)
    summary = _calculate_readiness(doc)

    if not summary["recovery_outcome_closure_finalisation_run_ready"]:
        frappe.throw(
            _("Cannot mark Ready for Closure Finalisation. Incomplete items: {0}").format(
                "; ".join(summary["incomplete"])
            )
        )

    doc.status = "Ready for Closure Finalisation"
    doc.readiness_percent = summary["readiness_percent"]
    doc.recovery_outcome_closure_finalisation_run_ready = 1
    doc.save(ignore_permissions=True)
    ...
    return {
        "doctype": RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN,
        "name": doc.name,
        "message": "Recovery Outcome Closure Finalisation Run marked Ready.",
    }


@frappe.whitelist()
def approve_recovery_outcome_closure_finalisation_run(recovery_outcome_closure_finalisation_run):
    _check_role()
    ...
    if not summary["recovery_outcome_closure_finalisation_run_ready"]:
        frappe.throw(
            _("Cannot approve Recovery Outcome Closure Finalisation Run. Incomplete items: {0}").format(
                "; ".join(summary["incomplete"])
            )
        )

    doc.status = "Closure Finalisation Approved"
    doc.readiness_percent = summary["readiness_percent"]
    doc.recovery_outcome_closure_finalisation_run_ready = 1
    ...
    for row in doc.get("recovery_outcome_closure_finalisation_lines") or []:
        if row.get("recovery_outcome_closure_finalisation_line_status") in ["Draft", "Ready"]:
            row.recovery_outcome_closure_finalisation_line_status = "Approved"
    ...
    return {
        "doctype": RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN,
        "name": doc.name,
        "message": "Recovery Outcome Closure Finalisation Run approved. No recovery case closure or downstream document was created.",
    }
...
def _ready_lines_for_finalisation(doc):
    ...
    for row in _active_lines(doc):
        if row.get("recovery_outcome_closure_finalisation_line_status") not in ["Approved", "Ready"]:
            continue

        if not row.get("line_ready_for_recovery_outcome_closure_finalisation"):
            continue

        if not row.get("recovery_outcome_closure_finalisation_source_ready"):
            continue

        if not row.get("closure_finalisation_review_complete"):
            continue

        if not row.get("closure_finalisation_authorized"):
            continue

        if not row.get("closure_finalisation_recorded"):
            continue
...
@frappe.whitelist()
def finalise_recovery_outcome_closure(recovery_outcome_closure_finalisation_run):
    _check_role()

    doc = frappe.get_doc(RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN, recovery_outcome_closure_finalisation_run)

    if doc.status != "Closure Finalisation Approved":
        frappe.throw(_("Recovery Outcome Closure Finalisation Run must be approved before finalisation."))

    if not doc.get("closure_finalisation_allowed"):
        frappe.throw(_("Tick Closure Finalisation Allowed before finalising closure."))

    if (doc.get("target_closure_finalisation_mode") or "CRM Closure Finalisation Only") != "CRM Closure Finalisation Only":
        frappe.throw(_("Phase 53 supports CRM Closure Finalisation Only. Actual recovery case closure is blocked until a later explicit gate."))
    ...
    for label, fieldname in blocked_run_fields:
        if doc.get(fieldname):
            frappe.throw(_("{0} must remain unticked in Phase 53.").format(label))

    summary = _calculate_readiness(doc)

    if not summary["recovery_outcome_closure_finalisation_run_ready"]:
        frappe.throw(
            _("Cannot finalise recovery outcome closure. Incomplete items: {0}").format(
                "; ".join(summary["incomplete"])
            )
        )
    ...
    if not ready_lines and not doc.get("no_recovery_outcome_closure_finalisation_candidate_found"):
        frappe.throw(_("No ready recovery outcome closure finalisation lines found."))

    for row in ready_lines:
        row.target_finalisation_doctype = RECOVERY_OUTCOME_CLOSURE_FINALISATION_LINE
        row.target_finalisation_name = row.name
        row.target_finalisation_status = "CRM Closure Finalised"
        row.recovery_outcome_closure_finalisation_line_status = "CRM Closure Finalised"

    doc.status = "Closure Finalised"
    doc.closure_finalisation_allowed = 0
    doc.actual_recovery_case_closure_allowed = 0
    doc.communication_creation_allowed = 0
    doc.email_send_allowed = 0
    ...
    return {
        ...
        "message": "Recovery outcome closure finalised in CRM-only control layer. No Recovery Case was closed. No Communication, Email Queue, Event, ToDo, Task, Journal Entry, GL Entry, Payment Entry, Sales Invoice, adjustment, bank reconciliation, Claim Batch, Claim Line, or Remittance Import was created.",
    }


def validate_recovery_outcome_closure_finalisation_run(doc, method=None):
    summary = _calculate_readiness(doc)
    _sync_totals(doc)
    ...
    if doc.status in READY_STATUSES and not summary["recovery_outcome_closure_finalisation_run_ready"]:
        frappe.throw(
            _("Cannot set Recovery Outcome Closure Finalisation Run to {0}. Incomplete items: {1}").format(
                doc.status,
                "; ".join(summary["incomplete"])
            )
        )

    if doc.status != "Closure Finalisation Approved" and doc.get("closure_finalisation_allowed"):
        frappe.throw(_("Closure Finalisation Allowed can only be ticked after the run is approved."))
    ...
    for label, fieldname in blocked_run_fields:
        if doc.get(fieldname):
            frappe.throw(_("{0} is not allowed in Phase 53.").format(label))


def on_recovery_outcome_closure_finalisation_run_update(doc, method=None):
    try:
        _sync_summary_to_links(doc)
    except Exception:
        frappe.log_error(
            frappe.get_traceback(),
            "NDIS CRM Recovery Outcome Closure Finalisation Run Summary Sync Failed"
        )


def validate_crm_deal_phase53(doc, method=None):
    if doc.status != "Won / Active Client":
        return

    required = 0

    if _field_exists(CRM_DEAL, "ndis_recovery_outcome_closure_finalisation_run_required"):
        required = doc.get("ndis_recovery_outcome_closure_finalisation_run_required")

    if not required:
        return

    run = doc.get("ndis_recovery_outcome_closure_finalisation_run") if _field_exists(CRM_DEAL, "ndis_recovery_outcome_closure_finalisation_run") else None

    if not run:
        frappe.throw(
            _("Cannot mark CRM Deal as Won / Active Client. NDIS Recovery Outcome Closure Finalisation Run must be created and approved/completed.")
        )

    if not _is_closure_finalisation_approved(run):
        frappe.throw(
            _("Cannot mark CRM Deal as Won / Active Client. NDIS Recovery Outcome Closure Finalisation Run must be approved/completed.")
        )


def validate_crm_deal_phase53_combined(doc, method=None):
    """
    Preserve Phase 2-52 validator chain, then add optional Phase 53 Recovery Outcome Closure Finalisation validation.
    """
    try:
    ...
    validate_crm_deal_phase53(doc, method)


def phase53_health_check():
    print("---- NDIS CRM Phase 53 Health Check ----")
    ...
    print(
        "NDIS CRM Recovery Outcome Closure Finalisation Run records:",
        frappe.db.count(RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN) if _doctype_exists(RECOVERY_OUTCOME_CLOSURE_FINALISATION_RUN) else 0
    )
    print("Phase 53 creates CRM recovery outcome closure finalisation run/line records only.")
    print("Phase 53 finalises CRM-only closure status inside the controlled chain.")
    print("Phase 53 does not close Recovery Case and does not create Communication, Email Queue, Event, ToDo, Task, Recovery Case, Journal Entry, GL Entry, Payment Entry, Sales Invoice, adjustment, bank reconciliation, Claim Batch, Claim Line, or NDIS Remittance Import.")
    print("---- End Phase 53 Health Check ----")
